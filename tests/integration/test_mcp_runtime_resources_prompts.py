...
class TestRegisterRuntimeResources:
    """Tests for register_runtime_resources (dynamic runtime stats)."""

    @pytest.mark.asyncio
    async def test_registers_runtime_stats_resource(self) -> None:
        server = create_runtime_server()
        ir = _build_test_ir()
        register_runtime_resources(server, ir)

        resources = await server.list_resources()
        uris = [str(r.uri) for r in resources]
        assert any("runtime-stats" in u for u in uris)

    @pytest.mark.asyncio
    async def test_runtime_stats_returns_valid_json(self) -> None:
        import json
        ...
        resources = await server.list_resources()
        stats_r = next(r for r in resources if "runtime-stats" in str(r.uri))
        content = list(await server.read_resource(stats_r.uri))
        parsed = json.loads(content[0].content)
        assert parsed["service_name"] == "test-service"
        assert parsed["protocol"] == "openapi"
        assert parsed["tool_count"] == 2

    @pytest.mark.asyncio
    async def test_runtime_stats_includes_observability_metrics(self) -> None:
        import json
        ...
        obs.register_operation("list_items")
        obs.record_tool_call("list_items", "success")
        obs.record_tool_call("list_items", "success")
        obs.record_tool_call("list_items", "error")

        register_runtime_resources(server, ir, observability=obs)

        resources = await server.list_resources()
        stats_r = next(r for r in resources if "runtime-stats" in str(r.uri))
        content = list(await server.read_resource(stats_r.uri))
        parsed = json.loads(content[0].content)
        assert "operation_metrics" in parsed
        assert "list_items" in parsed["operation_metrics"]

    @pytest.mark.asyncio
    async def test_runtime_stats_without_observability(self) -> None:
        import json

        server = create_runtime_server()
        ir = _build_test_ir()
        register_runtime_resources(server, ir, observability=None)

        resources = await server.list_resources()
        stats_r = next(r for r in resources if "runtime-stats" in str(r.uri))
        content = list(await server.read_resource(stats_r.uri))
        parsed = json.loads(content[0].content)
        assert "operation_metrics" not in parsed
