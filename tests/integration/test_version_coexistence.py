...
async def test_versioned_deployments_coexist_and_active_switch_is_atomic(
    ...
    assert manifest_v1.route_config["version_route"]["route_id"] == "billing-api-48be48f7-v1"
    assert manifest_v2.route_config["version_route"]["route_id"] == "billing-api-48be48f7-v2"
    ...
    async with session_factory() as session:
        ...
        assert active_before.route_config["default_route"]["target_service"]["host"] == (
            "billing-runtime-v1"
        )
        assert inactive_before.route_config["version_route"]["target_service"]["host"] == (
            "billing-runtime-v2"
        )
    ...
    assert activated.route_config["default_route"]["target_service"]["host"] == (
        "billing-runtime-v2"
    )
    assert preserved_old_version.is_active is False
    assert preserved_old_version.route_config["version_route"]["target_service"]["host"] == (
        "billing-runtime-v1"
    )
