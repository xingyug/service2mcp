...
async def test_default_activity_registry_executes_full_pipeline_with_runtime_harness(
    ...
    try:
        ...
        assert active.route_config["default_route"]["route_id"] == "petstore-api-61687e8c-active"
...
@pytest.mark.asyncio
async def test_force_protocol_overrides_auto_detected_context_protocol(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """force_protocol hint must win even when detect stage already set context.protocol."""
    registry = create_default_activity_registry(session_factory=session_factory)
    ...
    request = CompilationRequest(
        source_url="https://example.com/spec.txt",
        source_content="not-a-real-spec",
        service_name="force-override",
        options={"force_protocol": "soap"},
    )
    context = CompilationContext(
        job_id=request.job_id or uuid4(),
        request=request,
        payload={},
        protocol="rest",  # Simulate detect stage already set this
    )

    with (
        patch(
            "apps.compiler_worker.activities.production._build_extractors",
            return_value=[_DummyExtractor("rest"), _DummyExtractor("soap")],
        ),
        patch(
            "apps.compiler_worker.activities.production.TypeDetector.detect",
            side_effect=AssertionError("detection should not run"),
        ),
    ):
        result = await registry.run_stage(CompilationStage.EXTRACT, context)

    assert result.protocol == "soap"


@pytest.mark.asyncio
async def test_detect_stage_respects_force_protocol(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    """Detect stage should short-circuit when force_protocol is set."""
    registry = create_default_activity_registry(session_factory=session_factory)

    request = CompilationRequest(
        source_url="https://example.com/spec.wsdl",
        source_content="<definitions/>",
        service_name="detect-forced",
        options={"force_protocol": "soap"},
    )
    context = CompilationContext(
        job_id=request.job_id or uuid4(),
    ...
    with patch(
        "apps.compiler_worker.activities.production.TypeDetector.detect",
        side_effect=AssertionError("detection should not run when forced"),
    ):
        result = await registry.run_stage(CompilationStage.DETECT, context)

    assert result.protocol == "soap"
    assert result.context_updates["detection_confidence"] == 1.0
    ...
    async with httpx.AsyncClient(
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                assert "petstore-api-61687e8c-active" in routes
                assert "petstore-api-61687e8c-v1" in routes
                active_route = routes["petstore-api-61687e8c-active"]
                assert active_route["document"]["target_service"]["name"] == ("petstore-api-v1")
            finally:
                await deployment_harness.aclose()
    ...
    async with httpx.AsyncClient(
        ...
        async with httpx.AsyncClient(
            ...
            try:
                ...
                active_route = routes["petstore-api-61687e8c-active"]
                assert active_route["document"]["target_service"]["name"] == ("petstore-api-v1")
                assert "petstore-api-61687e8c-v2" not in routes
            finally:
                await deployment_harness.aclose()
