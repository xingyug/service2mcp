"""Large-surface pilot integration tests.
...
async def test_large_surface_rest_pilot_measures_three_coverage_numbers(
    ...
    """Pilot: measure discovery, generation, and audit coverage on 62 endpoints."""
    ...
    print(f"\n{'=' * 60}")
    print("LARGE-SURFACE PILOT REPORT")
    print(f"{'=' * 60}")
...
async def test_large_surface_pilot_p1_features(tmp_path: Path) -> None:
    """P1: exercise LLM seed mutation, grouping, bifurcation, and judge on pilot IR."""
    ...
    # --- Phase 5: Report ---
    print(f"\n{'=' * 60}")
    print("P1 FEATURES PILOT REPORT")
    print(f"{'=' * 60}")
...
async def test_large_surface_openapi_spec_first_pilot(tmp_path: Path) -> None:
    """Spec-first pilot: measure extraction, generation, and audit coverage
...
async def test_large_surface_pilot_p1_proof_runner_integration(
    ...
    """ verify tool_intent_counts and judge_evaluation flow through
    ...
    # --- Phase 5: Report ---
    print(f"\n{'=' * 60}")
    print("PROOF RUNNER INTEGRATION REPORT")
    print(f"{'=' * 60}")
