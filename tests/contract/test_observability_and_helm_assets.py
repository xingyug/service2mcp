...
def test_helm_chart_contains_core_metadata_and_hooked_migration_job() -> None:
    ...
    assert values["resources"]["compilerWorker"]["requests"]["memory"] == "512Mi"
    assert values["resources"]["temporal"]["limits"]["memory"] == "512Mi"
    ...
    assert ".Values.resources.compilerApi" in apps_template
    assert ".Values.resources.accessControl" in apps_template
    assert ".Values.resources.gatewayAdminMock" in apps_template
    assert ".Values.resources.compilerWorker" in apps_template
    assert ".Values.resources.mcpRuntime" in apps_template
    ...
    assert ".Values.resources.postgres" in infra_template
    assert ".Values.resources.redis" in infra_template
    assert ".Values.resources.temporal" in infra_template
