...
**Current best real-target image set:** For any next rerun, prefer `compiler-api:20260329-realproof-r8`, `compiler-worker:20260329-realproof-r10`, and `mcp-runtime:20260329-realproof-runtime-r4`. Keep LLM disabled unless the next task explicitly wants enhancer/judge coverage. Note that the `r10` worker behavior and the updated docs are still local worktree changes and have not yet been committed.
**Real-target endpoint coverage audit (2026-03-28):** A systematic audit compared every compiled MCP server tool against the original service's actual endpoints. The `realr12all` run produced 7 compiled MCP servers; 4 additional proof cases failed at `invocation_smoke`. Coverage varies drastically by protocol:

| Service | Protocol | Tools | Actual Endpoints | Coverage | Root Cause |
|---------|----------|-------|-----------------|----------|------------|
| Directus GraphQL | graphql | 33 | ~30+ | ✅ High | Introspection returns full schema |
| Northbreeze OData | odata | 17 | 3 EntitySets + func + action | ✅ High | `$metadata` returns full EDM |
| PostgreSQL | sql | 22 | 10 tables + 2 views × CRUD | ⚠️ Mid | `sql.py` only generates query + insert — no UPDATE/DELETE |
| aria2 | jsonrpc | 3 | 36 RPC methods | 🔴 8% | Manual 3-method definition; extractor lacks `system.listMethods` |
| OpenFGA | grpc | 1 | ~15+ RPCs | 🔴 7% | Manual 1-method proto; extractor lacks gRPC reflection |
| Directus REST | rest | 2 | many `/items/*` endpoints | 🔴 Minimal | `source_url` targets single collection; JSON responses have no HATEOAS links for BFS crawler |
| PocketBase REST | rest | 2 | collections CRUD | 🔴 Minimal | Same as Directus REST |

**Root cause classification:** (A) Extractor capability gaps — `sql.py` missing UPDATE/DELETE builders; `jsonrpc.py` requires OpenRPC spec without `system.listMethods` fallback; `grpc.py` uses regex proto parsing without gRPC reflection; `rest.py` BFS crawler ineffective on pure-JSON APIs. (B) Proof case config — REST `source_url` too narrow (single collection instead of API root); `llm_seed_mutation: false` disables LLM-assisted path discovery; aria2/OpenFGA use hardcoded minimal service definitions. (C) 4 invocation_smoke failures — likely auth propagation or runtime proxy issues, need log-level diagnosis.

**Priority improvements identified:** (1) gRPC reflection in `grpc.py` — standard protocol, benefits all reflection-enabled services. (2) `system.listMethods` in `jsonrpc.py` — simple, universal JSON-RPC auto-discovery. (3) REST wider entry points + `llm_seed_mutation: true` — config + minor code changes. (4) SQL UPDATE/DELETE in `sql.py` — add `_build_update_operation()` and `_build_delete_operation()`.

**Next up:** `B-008` Auth-Aware Discovery, Rate-Limit Backoff, Pagination — planned, not started. Previous tracks complete: `B-006` Enterprise Protocol Runtime Completion (OData/SCIM/JSON-RPC runtime adapters with request prep, response unwrap, error detection). `B-007` Toolchain Migration (uv + nox + basedpyright + pre-commit + CI update). 1321 tests, ruff/mypy/basedpyright clean.
