...
**Repository state:** The working tree tracks the private GitHub repository `xingyug/service2mcp` on branch `main`. Latest pushed documentation baseline at this snapshot: `f8edd46` (`docs: record real-target green baseline`), preceded by `34e5b2a` (`fix: harden real-target proof pipeline`). Run `make gitleaks` before each push.
**Real-target external API validation:** The full non-LLM real-target matrix is now green. After `realr22fix4` closed the original four blockers (`directus-openapi`, `gitea-openapi`, `jackson-scim`, `soap-cxf`) and `realr23directus` removed the residual Directus `getComment` path-param audit failure, the complete rerun `realr24all` finished **11/11 passed** with `compiler-api:20260329-realproof-r8` (`sha256:57084f40efd1df2b8a44628b54fe528e55de934db670c37c7f7e63f1694c0d85`), `compiler-worker:20260329-realproof-r10` (`sha256:4952ced511647a9e76286f4265f9e57617bab0b7eae1a19a43d52c2af56ba251`), and `mcp-runtime:20260329-realproof-runtime-r4` (`sha256:5a91920c88da30fcc429501a0b28d8a74b533554435b5d1dc8c52a1e48aecc4e`) with `ENABLE_LLM_ENHANCEMENT=0 ENABLE_LLM_JUDGE=0`. The aggregate audit for `realr24all` was **124 passed / 0 failed / 529 skipped / 653 generated** across: Directus GraphQL `12/0/21/33`, Directus REST `2/0/0/2`, Directus OpenAPI `33/0/121/154`, Gitea OpenAPI `49/0/357/406`, Aria2 JSON-RPC `3/0/0/3`, OpenFGA gRPC `1/0/0/1`, NorthBreeze OData `7/0/10/17`, PocketBase REST `2/0/0/2`, Jackson SCIM `2/0/8/10`, SOAP CXF `1/0/2/3`, and PostgreSQL SQL `12/0/10/22`.
**Latest real-target hardening:** `compiler-worker:20260329-realproof-r10` added an OpenAPI extractor fallback that infers any missing `{path_param}` directly from the URL template, restoring path params like `id` to IR even when the source spec omits them from the declared parameter list. Combined with the earlier non-LLM fixes already proven in `realr22fix4` and `realr23directus`, this removed the final active real-target audit failure and allowed the full matrix rerun to complete cleanly.
**Most important confirmed facts for the next agent:** there are no open real-target protocol blockers in the non-LLM path. The previously failing issues are now resolved as follows:
1. `directus-openapi`: proof-runner no longer over-samples optional params, feature-disabled `oauth` is skipped, and missing path-template params are restored into IR.
2. `gitea-openapi`: the earlier ConfigMap `422` and optional-query over-sampling are resolved; feature-disabled `getNodeInfo` is skipped.
3. `jackson-scim`: list operations are the correct proof surface for the current deployment; empty-directory `404`s on synthetic detail IDs are treated as skips rather than failures.
4. `soap-cxf`: runtime SOAP serialization now honors unqualified child elements required by the real CXF WSDL, so `GetOrderStatus` succeeds live.
**Current best real-target image set:** For any next rerun, prefer `compiler-api:20260329-realproof-r8`, `compiler-worker:20260329-realproof-r10`, and `mcp-runtime:20260329-realproof-runtime-r4`. Keep LLM disabled unless the next task explicitly wants enhancer/judge coverage.
**Real-target live coverage audit (2026-03-29):** After the green `realr24all` rerun, GKE was cleaned down to one latest live set only: the core `tool-compiler-llm-e2e` control-plane deployments plus the 11 `*-realr24all-v1` MCP runtimes. A follow-on live audit queried each runtime's `/tools`, compared it against the upstream live schema/discovery surface, and re-invoked one representative tool from inside `deployment/tool-compiler-compiler-api`.
...
| OpenFGA | grpc | 1 | Reflection exposed 19 OpenFGA RPCs (`21` including gRPC health) | 🔴 Partial (`1 / 19`) | `ListStores` → `ok` |
...
| PostgreSQL SQL | sql | 22 | Rich schema exists, but generated tools are query/insert oriented rather than full DML coverage | ⚠️ Partial database capability coverage | `query_categories` → `ok` |

**Coverage nuance:** Every one of the 11 latest runtimes is live-usable, because one representative tool invocation succeeded for each runtime. That does **not** prove every generated tool has already been exercised. The green `realr24all` proof audited the safe subset only: **124 passed / 0 failed / 529 skipped / 653 generated**. The biggest remaining structural gaps are now discovery-scope gaps rather than runtime correctness bugs: REST entry points are still too narrow for whole-service coverage, aria2 still lacks `system.listMethods` auto-discovery, OpenFGA still lacks reflection-driven extraction, and SQL still does not generate UPDATE/DELETE coverage.
**Planned next track:** `B-010` Real-Target Full Endpoint Coverage is now the active planning target. Its goal is to move from "11 runtimes are usable" to "each runtime covers the full visible upstream service surface" using the live `realr24all` matrix as the canonical acceptance set. The track is broken into nine ordered tasks in `docs/post-sdd-modular-expansion-plan.md`: coverage manifest capture; REST whole-service source expansion; REST auth/rate-limit/pagination hardening; JSON-RPC `system.listMethods`; gRPC reflection; SQL update/delete coverage; SCIM live-surface parity; regression locks for already-green protocols; then targeted reruns plus a final full rerun. The earlier `B-008` REST hardening items are not dropped, but are explicitly folded into `B-010-T2` and `B-010-T3` so the next work stays organized around endpoint parity instead of isolated extractor tweaks.

**Next up:** `B-010` Real-Target Full Endpoint Coverage — planned, not started. Supporting REST hardening from `B-008` is now intended to land inside the `B-010` execution order instead of being worked as a separate top-level milestone. Previous tracks complete: `B-006` Enterprise Protocol Runtime Completion (OData/SCIM/JSON-RPC runtime adapters with request prep, response unwrap, error detection). `B-007` Toolchain Migration (uv + nox + basedpyright + pre-commit + CI update). 1321 tests, ruff/mypy/basedpyright clean.
**Key files for Stream C — Enterprise Protocols (ENT-001–012):**
- `libs/ir/models.py` — `JsonRpcOperationConfig` class + `jsonrpc` field on `Operation` + coherence validator (updated existing grpc/soap/sql validators to also reject jsonrpc cross-combinations)
- `libs/extractors/odata.py` — `ODataExtractor`: OData v4 XML metadata parser, 5 CRUD operations per EntitySet, function/action imports; detect() inspects `$metadata` URL or `edmx:` XML namespace
- `libs/extractors/scim.py` — `SCIMExtractor`: SCIM 2.0 schema parser, attribute mutability rules (readOnly/immutable excluded from write ops); auth set to `none` (SCIM auth is deployment-time concern)
- `libs/extractors/jsonrpc.py` — `JsonRpcExtractor`: discovers methods from OpenRPC specs or manual `jsonrpc_service: true` fixtures; dots→underscores in operation IDs
- `libs/extractors/__init__.py` — exports all 9 extractors including ODataExtractor, SCIMExtractor, JsonRpcExtractor
- `libs/validator/capability_matrix.py` — added odata/scim/jsonrpc rows; `_CAPABILITY_ORDER` now has 11 entries
- `libs/validator/tests/test_capability_matrix.py` — updated expected 11-protocol order, added assertions for new protocols
- `apps/compiler_worker/activities/production.py` — `_build_extractors()` now includes all three new extractors (inserted before SQLExtractor)
- `apps/compiler_worker/tests/test_production_helpers.py` — **fixed**: `test_returns_nine_extractors` now correctly expects 9 extractors including odata, scim, jsonrpc
- `tests/integration/test_mcp_runtime_odata.py`, `test_mcp_runtime_scim.py`, `test_mcp_runtime_jsonrpc.py` — runtime integration tests (8 total, all passing)
**Known issues discovered during Stream C:**
- **Fixed:** `test_returns_six_extractors` → `test_returns_nine_extractors` with updated assertions for all 9 protocols.
- **Fixed:** 19 mypy strict-mode errors in `test_scim.py` (missing type annotations), `resource_generator.py` (type mismatch), `test_drift.py` (unused type: ignore), `test_examples_generator.py` (unparameterized `dict`).
- **MCP `$` parameter name stripping**: FastMCP strips the `$` prefix from parameter names when building JSON Schema for a tool (so `$filter` becomes `filter`). The proxy correctly re-adds `$` when reconstructing upstream OData query strings. Integration tests must call tools with `filter`/`select`/`top` (no `$`) while asserting upstream receives `$filter`/`$select`/`$top`. This is a FastMCP behavior, not a compiler bug, but it is surprising and worth documenting.
- **SCIM `base_url` default**: When no URL is supplied in `SourceConfig`, the SCIM extractor falls back to `"https://scim.example.com"` (placeholder). In production the operator must always supply a real tenant SCIM endpoint URL.
