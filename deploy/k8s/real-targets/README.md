...
Production-grade services for service2mcp protocol testing. These replace the lightweight
...
|----------|---------|-------|---------|-------------|
| GraphQL+REST+OpenAPI | Directus 11 | directus/directus:11.4.1 | 8055 | `directus.example-targets.svc.cluster.local:8055` |
| gRPC+HTTP | OpenFGA 1.8 | openfga/openfga:v1.8.2 | 8080,8081,3000 | `openfga.example-targets.svc.cluster.local` |
| JSON-RPC | aria2 | custom (alpine+aria2) | 6800 | `aria2.example-targets.svc.cluster.local:6800` |
| OData V4 | Northbreeze | custom (Python/Flask OData V4) | 4004 | `northbreeze.example-targets.svc.cluster.local:4004` |
| OpenAPI | Gitea 1.22 | gitea/gitea:1.22 | 3000 | `gitea.example-targets.svc.cluster.local:3000` |
| REST/JSON | PocketBase 0.25 | custom (alpine+pb) | 8090 | `pocketbase.example-targets.svc.cluster.local:8090` |
| SCIM | BoxyHQ Jackson | boxyhq/jackson:latest (26.2.0) | 5225 | `jackson.example-targets.svc.cluster.local:5225` |
| SOAP | Spring Boot CXF | custom (JRE 21) | 8080 | `soap-cxf.example-targets.svc.cluster.local:8080` |
| SQL | PostgreSQL 16 | postgres:16-alpine | 5432 | `real-postgres.example-targets.svc.cluster.local:5432` |

## Compiler Source URLs

``` 
GraphQL:   http://directus.example-targets.svc.cluster.local:8055/graphql
REST:      http://directus.example-targets.svc.cluster.local:8055/collections
OpenAPI:   http://directus.example-targets.svc.cluster.local:8055/server/specs/oas
gRPC:      grpc://openfga.example-targets.svc.cluster.local:8081
JSON-RPC:  http://aria2.example-targets.svc.cluster.local:6800/jsonrpc
OData:     http://northbreeze.example-targets.svc.cluster.local:4004/odata/v4/northbreeze/$metadata
OpenAPI:   http://gitea.example-targets.svc.cluster.local:3000/swagger.v1.json
REST:      http://pocketbase.example-targets.svc.cluster.local:8090/api/collections
SCIM:      http://jackson.example-targets.svc.cluster.local:5225/api/scim/v2.0/{tenant}/{product}
SOAP:      http://soap-cxf.example-targets.svc.cluster.local:8080/services/OrderService?wsdl
SQL:       postgresql://catalog:catalog@real-postgres.example-targets.svc.cluster.local:5432/catalog_v2
```
...
Built and pushed to `YOUR_REGISTRY/YOUR_PROJECT/`:
