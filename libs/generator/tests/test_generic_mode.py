...
def test_generate_generic_manifests_from_fixture() -> None:
    ...
    assert env_entries["APP_PORT"] == "8003"
    assert env_entries["TMPDIR"] == "/tmp"
    assert env_entries["MCP_ALLOWED_HOSTS"] == (
        "billing-runtime,"
        "billing-runtime:*,"
        "billing-runtime.runtime-system,"
        "billing-runtime.runtime-system:*,"
        "billing-runtime.runtime-system.svc,"
        "billing-runtime.runtime-system.svc:*,"
        "billing-runtime.runtime-system.svc.cluster.local,"
        "billing-runtime.runtime-system.svc.cluster.local:*"
    )
...
def test_generate_generic_manifests_supports_version_coexistence_route_config() -> None:
    ...
    assert route_config["service_id"] == "billing-api-48be48f7"
    assert route_config["service_name"] == "billing-runtime-v2"
    assert route_config["default_route"]["route_id"] == "billing-api-48be48f7-active"
    assert route_config["default_route"]["target_service"] == {
        "host": "billing-runtime-v2",
        "namespace": "runtime-system",
        "port": 8080,
    }
    assert route_config["version_route"] == {
        "route_id": "billing-api-48be48f7-v2",
...
def test_generate_generic_manifests_scopes_gateway_route_ids_by_service_scope() -> None:
    ...
    assert route_config["default_route"]["route_id"] == (
        "billing-api-48be48f7-tenant-team-a-env-prod-active"
    )
    assert route_config["version_route"]["route_id"] == (
        "billing-api-48be48f7-tenant-team-a-env-prod-v2"
    )
...
class TestGenericManifestConfigValidation:
    ...
    def test_rejects_container_port_less_than_one(self) -> None:
        with pytest.raises(ValueError, match="container_port must be between 1 and 65535"):
            GenericManifestConfig(runtime_image="img:latest", container_port=0)

    def test_rejects_service_port_less_than_one(self) -> None:
        with pytest.raises(ValueError, match="service_port must be between 1 and 65535"):
            GenericManifestConfig(runtime_image="img:latest", service_port=0)

    def test_rejects_container_port_above_65535(self) -> None:
        with pytest.raises(ValueError, match="container_port must be between 1 and 65535"):
            GenericManifestConfig(runtime_image="img:latest", container_port=70000)

    def test_rejects_service_port_above_65535(self) -> None:
        with pytest.raises(ValueError, match="service_port must be between 1 and 65535"):
            GenericManifestConfig(runtime_image="img:latest", service_port=100000)
...
def test_route_base_name_no_collision_after_sanitization() -> None:
    """collapsing DNS labels must produce different route names."""
    ir = _load_service_ir()
    config_a = GenericManifestConfig(runtime_image="img:latest", service_id="payment@api")
    config_b = GenericManifestConfig(runtime_image="img:latest", service_id="payment_api")
    assert _sanitize_dns_label("payment@api") == _sanitize_dns_label("payment_api")
    route_a = _route_base_name(ir, config_a)
    route_b = _route_base_name(ir, config_b)
