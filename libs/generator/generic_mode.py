...
def generate_generic_manifests(
    ...
    *,
    ...
    context = {
        ...
        "mcp_allowed_hosts": ",".join(_runtime_allowed_hosts(resource_name, config.namespace)),
...
def _route_base_name(service_ir: ServiceIR, config: GenericManifestConfig) -> str:
    original = config.service_id if config.service_id is not None else service_ir.service_name
    sanitized = _sanitize_dns_label(original)
    hash8 = hashlib.sha256(original.encode("utf-8")).hexdigest()[:8]
    return _sanitize_dns_label(f"{sanitized}-{hash8}")
...
def _runtime_allowed_hosts(resource_name: str, namespace: str) -> list[str]:
    cluster_hosts = (
        resource_name,
