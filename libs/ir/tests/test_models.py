...
class TestSlaConfig:
    def test_default_sla_is_none(self) -> None:
        """Operations default to no SLA config."""
        op = Operation(
            id="test",
            name="test",
            description="test",
    ...
    def test_sla_config_with_latency_budget(self) -> None:
        sla = SlaConfig(latency_budget_ms=500, timeout_ms=5000)
        assert sla.latency_budget_ms == 500
        assert sla.timeout_ms == 5000
        assert sla.retry.max_retries == 0

    def test_sla_config_with_retry(self) -> None:
        retry = RetryConfig(max_retries=3, backoff_base_ms=200, retryable_errors=["timeout", "502"])
        sla = SlaConfig(latency_budget_ms=1000, retry=retry)
        assert sla.retry.max_retries == 3
        assert sla.retry.backoff_base_ms == 200
        assert len(sla.retry.retryable_errors) == 2

    def test_retry_config_validation(self) -> None:
        with pytest.raises(Exception):
            RetryConfig(max_retries=-1)  # ge=0 constraint
        with pytest.raises(Exception):
            RetryConfig(max_retries=11)  # le=10 constraint

    def test_sla_config_latency_must_be_positive(self) -> None:
        with pytest.raises(Exception):
            SlaConfig(latency_budget_ms=0)  # gt=0 constraint
        with pytest.raises(Exception):
            SlaConfig(timeout_ms=-100)  # gt=0 constraint

    def test_sla_roundtrip_serialization(self) -> None:
        sla = SlaConfig(
            latency_budget_ms=500,
            timeout_ms=5000,
            retry=RetryConfig(max_retries=2, retryable_errors=["timeout"]),
        )
        data = sla.model_dump()
        restored = SlaConfig(**data)
        assert restored == sla

    def test_operation_with_sla(self) -> None:
        op = Operation(
            id="test",
            name="test",
            description="test",
            ...
            sla=SlaConfig(latency_budget_ms=200, timeout_ms=1000),
        )
