...
class PostDeployValidator:
    ...
    def _health_endpoint_failure_detail(endpoint: str, response: httpx.Response) -> str | None:
        ...
        if "status" not in payload:
            return f"{endpoint} response missing 'status' field"
...
def _tool_result_failure_reason(
    ...
    # "ok" — upstream returned a successful result.
    # "upstream_error" — MCP runtime reached the upstream but it returned a
    # protocol-level error (SOAP fault, JSON-RPC error, HTTP 4xx business
    # error).  This still proves end-to-end connectivity so the smoke test
    # should pass.
