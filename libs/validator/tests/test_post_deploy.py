...
@pytest.mark.asyncio
async def test_post_deploy_health_check_missing_status_field_returns_failure() -> None:
    """Health check reports failure when JSON body has no 'status' key."""
    ...
    async def handler(request: httpx.Request) -> httpx.Response:
        ...
        if "/readyz" in path:
            return httpx.Response(200, json={"uptime": 42}, request=request)
        if "/tools" in path:
            return httpx.Response(200, json={"tools": []}, request=request)
        return httpx.Response(200, request=request)
    ...
    service_ir = ServiceIR(
        source_hash="e" * 64,
        protocol="openapi",
        service_name="health-missing-status",
        service_description="Health missing status field fixture",
        base_url="https://example.test",
        auth=AuthConfig(type=AuthType.none),
        operations=[],
    )
    ...
    assert report.get_result("health").passed is False
    assert "missing" in report.get_result("health").details.lower()
...
# ── upstream_error smoke test tolerance ──────────────────────────────


def test_tool_result_failure_reason_accepts_upstream_error() -> None:
    """upstream_error status should pass smoke test (proves connectivity)."""
    from libs.validator.post_deploy import _tool_result_failure_reason
    ...
    result = {"status": "upstream_error", "error": "SOAP fault: Order not found"}
    assert _tool_result_failure_reason(ir, "any_op", result) is None


def test_tool_result_failure_reason_rejects_generic_error() -> None:
    """Plain 'error' status (connection failure) should still fail."""
    from libs.validator.post_deploy import _tool_result_failure_reason
    ...
    reason = _tool_result_failure_reason(ir, "any_op", result)
