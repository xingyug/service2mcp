...
# ---------------------------------------------------------------------------
# CoverageExpectation / CoverageReport tests
# ---------------------------------------------------------------------------
...
class TestCoverageExpectation:
    def test_basic_fields(self) -> None:
        exp = CoverageExpectation(
            service_id="directus-graphql",
            protocol="graphql",
            expected_tools=33,
        )
        assert exp.service_id == "directus-graphql"
        assert exp.protocol == "graphql"
        assert exp.expected_tools == 33
        assert exp.expected_passed is None
        assert exp.expected_actions is None
        assert exp.notes == ""

    def test_optional_fields(self) -> None:
        exp = CoverageExpectation(
            service_id="postgres-sql",
            protocol="sql",
            expected_tools=44,
            expected_passed=10,
            expected_actions={"query": 12, "insert": 10, "update": 10, "delete": 10},
            notes="Full CRUD",
        )
        assert exp.expected_passed == 10
        assert exp.expected_actions == {"query": 12, "insert": 10, "update": 10, "delete": 10}
        assert exp.notes == "Full CRUD"

    def test_frozen(self) -> None:
        exp = CoverageExpectation(service_id="test", protocol="sql", expected_tools=10)
        try:
            exp.expected_tools = 20
...
class TestCoverageReport:
    def test_meets_expectation_when_above(self) -> None:
        exp = CoverageExpectation(service_id="svc", protocol="sql", expected_tools=10)
        report = CoverageReport(
            ...
            actual_failed=0,
            actual_skipped=2,
            actual_actions={"query": 6, "insert": 6},
            expectation=exp,
        )
        assert report.meets_expectation is True
        assert report.violations == []

    def test_meets_expectation_when_exact(self) -> None:
        exp = CoverageExpectation(service_id="svc", protocol="sql", expected_tools=10)
        report = CoverageReport(
            service_id="svc",
            protocol="sql",
            ...
            actual_failed=0,
            actual_skipped=2,
            actual_actions={"query": 5, "insert": 5},
            expectation=exp,
        )
        assert report.meets_expectation is True

    def test_fails_expectation_when_below(self) -> None:
        exp = CoverageExpectation(service_id="svc", protocol="sql", expected_tools=10)
        report = CoverageReport(
            service_id="svc",
            protocol="sql",
            ...
            actual_failed=0,
            actual_skipped=2,
            actual_actions={"query": 4, "insert": 4},
            expectation=exp,
        )
        assert report.meets_expectation is False
        assert len(report.violations) == 1
        assert "below expected 10" in report.violations[0]
    ...
    def test_no_expectation_always_passes(self) -> None:
        report = CoverageReport(
            ...
            actual_failed=0,
            actual_skipped=2,
            actual_actions={"query": 5},
        )
        assert report.meets_expectation is True
        assert report.violations == []

    def test_passed_violation(self) -> None:
        exp = CoverageExpectation(
            service_id="svc", protocol="sql", expected_tools=10, expected_passed=8
        )
        report = CoverageReport(
            service_id="svc",
            protocol="sql",
            ...
            actual_failed=0,
            actual_skipped=5,
            actual_actions={"query": 10},
            expectation=exp,
        )
        assert report.meets_expectation is True
        violations = report.violations
        assert len(violations) == 1
        assert "Passed count 5" in violations[0]

    def test_action_violation(self) -> None:
        exp = CoverageExpectation(
            service_id="svc",
            protocol="sql",
            expected_tools=10,
            expected_actions={"query": 5, "insert": 5},
        )
        report = CoverageReport(
            service_id="svc",
            protocol="sql",
            ...
            actual_failed=0,
            actual_skipped=2,
            actual_actions={"query": 5, "insert": 3},
            expectation=exp,
        )
        violations = report.violations
        assert len(violations) == 1
        assert "Action 'insert'" in violations[0]
...
class TestCheckCoverage:
    ...
    def test_all_passing(self) -> None:
        exp = CoverageExpectation(service_id="a", protocol="sql", expected_tools=5)
        r = CoverageReport(
            service_id="a",
            protocol="sql",
            actual_tools=5,
    ...
    def test_mixed_pass_fail(self) -> None:
        exp_pass = CoverageExpectation(service_id="a", protocol="sql", expected_tools=5)
        exp_fail = CoverageExpectation(service_id="b", protocol="grpc", expected_tools=20)
        r_pass = CoverageReport(
            service_id="a",
            protocol="sql",
            actual_tools=5,
            actual_passed=5,
            ...
            actual_skipped=0,
            actual_actions={"query": 5},
            expectation=exp_pass,
        )
        r_fail = CoverageReport(
            service_id="b",
            protocol="grpc",
            actual_tools=10,
            actual_passed=8,
            ...
            actual_skipped=2,
            actual_actions={},
            expectation=exp_fail,
        )
        violations = check_coverage([r_pass, r_fail])
        assert len(violations) == 1
        assert "below expected 20" in violations[0]
