...
class CoverageExpectation:
    """Declared coverage expectation for a real-target service.

    Used by regression locks to assert that a compiled service produces
    the expected number of tools for each CRUD action category.
    """
    ...
    expected_actions: dict[str, int] | None = None
    notes: str = ""


@dataclass(frozen=True)
class CoverageReport:
    """Observed coverage for a single service after compilation and audit.

    Compares actual tool counts against a ``CoverageExpectation`` to
    produce a pass/fail verdict with per-action breakdowns.
    """
    ...
    actual_failed: int
    actual_skipped: int
    actual_actions: dict[str, int]
    expectation: CoverageExpectation | None = None

    @property
    def meets_expectation(self) -> bool:
        """Return True if actual tool count meets or exceeds expectation."""
        if self.expectation is None:
            return True
        return self.actual_tools >= self.expectation.expected_tools

    @property
    def violations(self) -> list[str]:
        """Return a list of expectation violation messages."""
        if self.expectation is None:
            return []
        messages: list[str] = []
        if self.actual_tools < self.expectation.expected_tools:
            messages.append(
                f"Tool count {self.actual_tools} is below expected "
                f"{self.expectation.expected_tools} for {self.service_id}."
            )
        if (
            self.expectation.expected_passed is not None
            and self.actual_passed < self.expectation.expected_passed
        ):
            messages.append(
                f"Passed count {self.actual_passed} is below expected "
                f"{self.expectation.expected_passed} for {self.service_id}."
            )
        if self.expectation.expected_actions is not None:
            for action, expected_count in self.expectation.expected_actions.items():
                actual_count = self.actual_actions.get(action, 0)
                if actual_count < expected_count:
                    messages.append(
                        f"Action '{action}' count {actual_count} is below expected "
                        f"{expected_count} for {self.service_id}."
                    )
        return messages
...
def check_coverage(
    ...
    """Check a list of coverage reports against their expectations.

    Returns a list of human-readable violation messages.  An empty list
    means all expectations are met.
    """
    violations: list[str] = []
...
class LargeSurfacePilotReport:
    ...
    Captures the three core coverage numbers plus unsupported patterns
