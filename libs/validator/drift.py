...
class DriftSeverity(enum.StrEnum):
    """Severity classification of a drift finding."""
...
def _compare_operation(deployed_op: Any, live_op: Any) -> list[str]:
    ...
    # Compare SLA configuration
    deployed_sla = getattr(deployed_op, "sla", None)
    live_sla = getattr(live_op, "sla", None)
    if deployed_sla != live_sla:
        if deployed_sla is None and live_sla is not None:
            changes.append("sla config added")
        elif deployed_sla is not None and live_sla is None:
            changes.append("sla config removed")
        else:
            # Both exist, compare details
            assert deployed_sla is not None
            assert live_sla is not None
            if deployed_sla.latency_budget_ms != live_sla.latency_budget_ms:
                old_budget = deployed_sla.latency_budget_ms
                new_budget = live_sla.latency_budget_ms
                if old_budget is None:
                    changes.append(f"sla latency budget added: {new_budget}ms")
                elif new_budget is None:
                    changes.append(f"sla latency budget removed (was {old_budget}ms)")
                else:
                    direction = "tightened" if new_budget < old_budget else "relaxed"
                    changes.append(
                        f"sla latency budget {direction}: {old_budget}ms -> {new_budget}ms"
                    )
            if deployed_sla.timeout_ms != live_sla.timeout_ms:
                old_timeout = deployed_sla.timeout_ms
                new_timeout = live_sla.timeout_ms
                if old_timeout is None:
                    changes.append(f"sla timeout added: {new_timeout}ms")
                elif new_timeout is None:
                    changes.append(f"sla timeout removed (was {old_timeout}ms)")
                else:
                    direction = "tightened" if new_timeout < old_timeout else "relaxed"
                    changes.append(f"sla timeout {direction}: {old_timeout}ms -> {new_timeout}ms")
            if deployed_sla.retry != live_sla.retry:
                changes.append("sla retry config changed")
...
def _classify_severity(change: str) -> DriftSeverity:
    """Classify a single change description as breaking or non-breaking."""
    lower = change.lower()
    for pattern in _BREAKING_PATTERNS:
        if pattern in lower:
            return DriftSeverity.breaking
    return DriftSeverity.non_breaking
...
def detect_drift(deployed_ir: ServiceIR, live_ir: ServiceIR) -> DriftReport:
    ...
    for op_id in sorted(deployed_ops.keys() & live_ops.keys()):
        ...
        if changes:
            op_severity = max(
                (_classify_severity(c) for c in changes),
                key=lambda s: s == DriftSeverity.breaking,
            )
            modified.append(DriftDetail(operation_id=op_id, changes=changes, severity=op_severity))

    schema_changes = _compare_schema(deployed_ir, live_ir)

    has_drift = bool(added or removed or modified or schema_changes)

    # Overall severity: breaking if any removed ops, any breaking modifications,
    # or any breaking schema changes.
    overall_severity = DriftSeverity.non_breaking
    if removed:
        overall_severity = DriftSeverity.breaking
    elif any(m.severity == DriftSeverity.breaking for m in modified):
        overall_severity = DriftSeverity.breaking
    elif any(_classify_severity(c) == DriftSeverity.breaking for c in schema_changes):
        overall_severity = DriftSeverity.breaking
