...
def test_json_body_values_not_promoted_to_endpoints() -> None:
    ...
    Reproduces a live audit failure where 5 of 6 generated REST tools failed because
    they pointed at spurious paths derived from JSON field values.
    """
...
def test_relative_json_links_are_still_discovered() -> None:
    """Regression: link-like relative JSON paths must still be discovered
    after sibling coalesce refactor."""
...
class TestAuthHeaders:
    ...
    def test_auth_header_and_token_combined(self) -> None:
        """When both provided, auth_header is the header name and auth_token the value."""
        transport = httpx.MockTransport(lambda r: httpx.Response(404, request=r))
        extractor = RESTExtractor(client=httpx.Client(transport=transport))
        source = SourceConfig(
            url="https://x.com",
...
class TestParseLinkHeaderNext:
    """Tests for RFC 5988 Link header parsing."""
    ...
    def test_simple_next_link(self) -> None:
        ...
        result = _parse_link_header_next(response, "https://api.example.com/items")
    ...
    def test_multiple_rels(self) -> None:
        ...
        result = _parse_link_header_next(response, "https://api.example.com/items")
        assert result == "https://api.example.com/items?page=3"

    def test_no_next_rel(self) -> None:
        response = httpx.Response(
            200,
            headers={"Link": '<https://api.example.com/items?page=1>; rel="prev"'},
        )
        result = _parse_link_header_next(response, "https://api.example.com/items")
        assert result is None
    ...
    def test_cross_origin_rejected(self) -> None:
        response = httpx.Response(
            200,
            headers={"Link": '<https://evil.com/items?page=2>; rel="next"'},
        )
        result = _parse_link_header_next(response, "https://api.example.com/items")
        assert result is None
...
class TestExtractRateLimitInfo:
    """Tests for X-RateLimit-* and RateLimit-* header parsing."""

    def test_standard_x_ratelimit_headers(self) -> None:
        response = httpx.Response(
            200,
            headers={
    ...
    def test_ietf_draft_headers(self) -> None:
        response = httpx.Response(
            200,
            headers={
                "RateLimit-Limit": "50",
    ...
    def test_no_ratelimit_headers(self) -> None:
        response = httpx.Response(200)
        info = extract_rate_limit_info(response)
        assert info is None
...
# ---------------------------------------------------------------------------
# Auth requirement detection tests
# ---------------------------------------------------------------------------


class TestDetectAuthRequirements:
    """Tests for WWW-Authenticate header parsing from 401 responses."""
    ...
    def test_bearer_auth(self) -> None:
        ...
        result = detect_auth_requirements(response)
        assert result is not None
    ...
    def test_non_401_returns_none(self) -> None:
        response = httpx.Response(200)
        assert detect_auth_requirements(response) is None

    def test_401_without_www_authenticate(self) -> None:
        response = httpx.Response(401)
        result = detect_auth_requirements(response)
        assert result is not None
        assert result["scheme"] == "unknown"

    def test_401_with_negotiate(self) -> None:
        response = httpx.Response(
            401,
        ...
        result = detect_auth_requirements(response)
        assert result is not None
        assert result["scheme"] == "negotiate"

    def test_401_with_empty_www_authenticate(self) -> None:
        response = httpx.Response(401, headers={"WWW-Authenticate": ""})
        result = detect_auth_requirements(response)
        assert result is not None
        assert result["scheme"] == "unknown"

    def test_401_with_whitespace_www_authenticate(self) -> None:
        response = httpx.Response(401, headers={"WWW-Authenticate": "   "})
        result = detect_auth_requirements(response)
        assert result is not None
        assert result["scheme"] == "unknown"
...
class TestCandidatePathsSizeLimit:
    """Response body size limit for HTML parsing."""

    def test_oversized_response_returns_empty(self) -> None:
        """Responses > 5MB should not be parsed for candidate paths."""
        from libs.extractors.rest_probing import _extract_candidate_paths

        # Create a 6MB HTML body with links
        big_body = "<html><body>" + '<a href="/api/v1">link</a>' * 300_000 + "</body></html>"
        response = httpx.Response(
            200,
            content=big_body.encode(),
            headers={"content-type": "text/html"},
        )
        result = _extract_candidate_paths("https://example.com", response)
        assert result == []

    def test_normal_sized_response_works(self) -> None:
        """Normal-sized HTML responses should still be parsed."""
        from libs.extractors.rest_probing import _extract_candidate_paths

        body = '<html><body><a href="/api/v1">link</a></body></html>'
        response = httpx.Response(
            200,
            content=body.encode(),
            headers={"content-type": "text/html"},
