...
def test_extracts_unary_rpcs_and_skips_streaming_methods() -> None:
    ...
    assert service_ir.event_descriptors[0].support is EventSupportLevel.supported
...
def test_extract_succeeds_when_all_rpcs_are_streaming_with_server_stream() -> None:
    """extract() succeeds when server-streaming RPCs exist (on by default)."""
    extractor = GrpcProtoExtractor()
    content = """\
syntax = "proto3";
package test;
...
"""
    ...
    assert "Watch" in op_ids
    assert "Chat" not in op_ids
    assert ir.metadata["ignored_streaming_rpcs"] == ["Chat"]


def test_extract_raises_when_all_rpcs_are_streaming_and_disabled() -> None:
    """extract() raises ValueError when server-stream is disabled and no unary RPCs exist."""
    extractor = GrpcProtoExtractor()
    content = """\
syntax = "proto3";
package test;
    ...
    with pytest.raises(ValueError, match="No unary RPC methods found"):
        extractor.extract(
            SourceConfig(
                ...
                hints={"disable_grpc_server_stream": "true"},
            )
        )
