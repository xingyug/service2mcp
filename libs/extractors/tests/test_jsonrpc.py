...
def test_detect_live_probe_system_list_methods() -> None:
    """Live probe: POST system.listMethods returns method list → 0.85."""
    extractor = JsonRpcExtractor()
    source = SourceConfig(url="http://example.com/jsonrpc")
    mock_response = MagicMock()
    mock_response.status_code = 200
    mock_response.json.return_value = {
        "jsonrpc": "2.0",
        "result": ["system.listMethods", "getVersion"],
        "id": 1,
    }
    with (
        patch.object(extractor, "_get_content", return_value=None),
        patch("libs.extractors.jsonrpc.httpx.post", return_value=mock_response),
    ):
        assert extractor.detect(source) == 0.85


def test_detect_live_probe_jsonrpc_error_response() -> None:
    """Live probe: JSON-RPC error response (method not found) → 0.6."""
    extractor = JsonRpcExtractor()
    source = SourceConfig(url="http://example.com/jsonrpc")
    mock_response = MagicMock()
    mock_response.status_code = 200
...
def test_detect_live_probe_connection_failure() -> None:
    """Live probe: connection failure → 0.0 (graceful fallback)."""
    extractor = JsonRpcExtractor()
    source = SourceConfig(url="http://unreachable.local/jsonrpc")
    with (
        patch.object(extractor, "_get_content", return_value=None),
...
# ── params_type probing and fallback default ──────────────────────────


def test_fallback_params_type_defaults_to_positional() -> None:
    """Default fallback params_type is positional (most compatible)."""
    source = SourceConfig(url="http://example.com/jsonrpc")
    assert JsonRpcExtractor._fallback_params_type(source) == "positional"


def test_fallback_params_type_respects_hint() -> None:
    """Hint overrides the default."""
    source = SourceConfig(
        url="http://example.com/jsonrpc",
        hints={"jsonrpc_fallback_params_type": "named"},
    )
    assert JsonRpcExtractor._fallback_params_type(source) == "named"


def test_discover_methods_probes_params_type() -> None:
    """When no hint is set, _discover_methods probes the server to detect params style."""
    extractor = JsonRpcExtractor()
    source = SourceConfig(url="http://example.com/jsonrpc")

    call_count = 0
    calls_bodies: list[dict] = []

    def mock_post(url: str, *, json: dict, **kwargs: object) -> MagicMock:
        nonlocal call_count
        call_count += 1
        calls_bodies.append(json)
        resp = MagicMock()
        resp.status_code = 200
        resp.raise_for_status = MagicMock()
        # system.listMethods always returns a list
        resp.json.return_value = {"jsonrpc": "2.0", "result": ["echo"], "id": 1}
        return resp

    with (
        patch.object(extractor, "_get_content", return_value=None),
        patch("libs.extractors.jsonrpc.httpx.post", side_effect=mock_post),
    ):
        methods = extractor._discover_methods(source)

    assert len(methods) == 1
    assert methods[0]["name"] == "echo"
    # Should have called: discovery + probe (positional first)
    assert call_count >= 2
    # Stub method payload type should be "array" (positional)
    assert methods[0]["params"][-1]["schema"]["type"] == "array"


def test_discover_methods_detects_named_params_when_positional_fails() -> None:
    """If positional probe fails and named succeeds, should use named."""
    extractor = JsonRpcExtractor()
    source = SourceConfig(url="http://example.com/jsonrpc")
    ...
    def mock_post(url: str, *, json: dict, **kwargs: object) -> MagicMock:
        ...
        if isinstance(params, list):
            ...
            # Initial discovery (no params set)
            resp.status_code = 200
            resp.json.return_value = {"jsonrpc": "2.0", "result": ["echo"], "id": 1}
        return resp
    ...
    assert len(methods) == 1
    # Should auto-detect "named" since positional failed
    assert methods[0]["params"][-1]["schema"]["type"] == "object"
