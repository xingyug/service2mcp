...
def test_detect_live_probe_metadata_endpoint(extractor: ODataExtractor) -> None:
    """Live probe: GET {base}/$metadata returns EDMX → 0.85."""
    from unittest.mock import MagicMock, patch

    source = SourceConfig(url="http://example.com/odata")
    ...
    with (
        ...
        patch("libs.extractors.odata.httpx.get", return_value=mock_response),
    ):
        assert extractor.detect(source) == 0.85


def test_detect_live_probe_connection_failure(extractor: ODataExtractor) -> None:
    """Live probe: connection failure → 0.0."""
    from unittest.mock import patch

    import httpx

    source = SourceConfig(url="http://unreachable.local/odata")
    with (
        patch.object(extractor, "_get_content", return_value=None),
        patch("libs.extractors.odata.httpx.get", side_effect=httpx.ConnectError("boom")),
    ):
        assert extractor.detect(source) == 0.0
