...
class GrpcProtoExtractor:
    ...
    def extract(self, source: SourceConfig) -> ServiceIR:
        ...
        # Server-streaming: on by default (opt-out)
        disable_server_stream = _hint_enabled(source, "disable_grpc_server_stream")
        # Client/bidi streaming: off by default (opt-in, for advanced users)
        enable_client_stream = _hint_enabled(source, "enable_grpc_client_stream")
        # Backward compat: old hint overrides server-stream default
        old_hint_value = source.hints.get("enable_native_grpc_stream")
        if old_hint_value is not None:
            old_enabled = _hint_enabled(source, "enable_native_grpc_stream")
            if not old_enabled:
                disable_server_stream = True
        ...
        for service in services:
            for rpc in service.rpcs:
                if rpc.client_streaming or rpc.server_streaming:
                    ...
                    supported_native_stream = False
                    if grpc_stream_mode is GrpcStreamMode.server:
                        supported_native_stream = not disable_server_stream
                    elif grpc_stream_mode is GrpcStreamMode.client:
                        supported_native_stream = enable_client_stream
    ...
    return Operation(
        ...
        params=[
            Param(
                ...
                description="",
                json_schema=_json_schema_for_proto_field(field, messages=messages, enums=enums),
...
def _json_schema_for_proto_field(
    field: ProtoField,
    *,
    messages: dict[str, list[ProtoField]],
    enums: set[str],
) -> dict[str, Any] | None:
    """Build json_schema for a gRPC proto field when it carries structure."""
    type_name = _strip_qualification(field.type_name)

    if field.repeated:
        # Determine the scalar IR type of the item (ignoring repeated flag)
        if type_name in _PROTO_TYPE_MAP:
            item_type = _PROTO_TYPE_MAP[type_name]
        ...
        if item_type == "object" and type_name in messages:
            item_schema = _response_schema_for_fields(
                messages[type_name], messages=messages, enums=enums
            )
            if item_schema:
                return {"type": "array", "items": item_schema}
        return {"type": "array", "items": {"type": item_type}}

    # Non-repeated complex type
    if type_name in messages:
        return _response_schema_for_fields(messages[type_name], messages=messages, enums=enums)
    return None
