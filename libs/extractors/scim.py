...
class SCIMExtractor:
    ...
    def extract(self, source: SourceConfig) -> ServiceIR:
        ...
        for resource in resources:
            ...
            if not isinstance(name, str) or not name:
                logger.warning(
                    "SCIM resource without valid 'name' field skipped: %s",
    ...
    def _extract_resources(self, data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        Only returns items that look like SCIM schema definitions (have string
        ``name`` and ``attributes`` list), not raw user/group records which also
        appear under ``Resources`` in a SCIM ListResponse.
        """
    ...
    def _attrs_to_params(self, attrs: list[dict[str, Any]]) -> list[Param]:
        return [
            Param(
                ...
                json_schema=self._build_complex_json_schema(a),
            )
            for a in attrs
        ]

    def _build_complex_json_schema(self, attr: dict[str, Any]) -> dict[str, Any] | None:
        """Build json_schema for SCIM complex attributes with subAttributes."""
        scim_type = attr.get("type", "string")
        sub_attrs = attr.get("subAttributes", [])
        if scim_type != "complex" or not sub_attrs:
            return None
        ...
        for sa in sub_attrs:
            ...
            if not sa_name:
                continue
            sa_type = SCIM_TYPE_MAP.get(sa.get("type", "string"), "string")
            entry: dict[str, Any] = {"type": sa_type}
            sa_desc = sa.get("description")
            if sa_desc:
                entry["description"] = str(sa_desc)
            properties[sa_name] = entry
            if sa.get("required", False):
                required.append(sa_name)
        ...
        if multi:
            return {"type": "array", "items": obj_schema}
        return obj_schema
