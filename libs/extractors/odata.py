...
class ODataExtractor:
    ...
    def detect(self, source: SourceConfig) -> float:
        ...
        # Live probe: try fetching $metadata from the base URL — many OData
        # services expose it at {base}/$metadata even when the base URL itself
        # returns plain HTML or a service document.
        if source.url and not path_or_url.endswith("$metadata"):
            metadata_url = source.url.rstrip("/") + "/$metadata"
            try:
                response = httpx.get(
                    metadata_url,
                    timeout=5,
                ...
                if response.status_code == 200 and "<edmx:Edmx" in response.text:
                    return 0.85
            except Exception:
                pass
