...
def _infer_scalar_type(name: str) -> str:
    """Infer IR type from unknown custom scalar name by naming convention."""
    lower = name.lower()
    if any(kw in lower for kw in ("json", "object", "map", "dict")):
        return "object"
    float_kw = ("float", "double", "decimal", "price", "amount", "rate", "percent")
    if any(kw in lower for kw in float_kw):
        return "number"
    if any(kw in lower for kw in ("int", "long", "count", "size", "port")):
        return "integer"
    if any(kw in lower for kw in ("bool", "flag", "enabled", "active")):
        return "boolean"
    return "string"
...
class GraphQLExtractor:
    ...
    def _param_from_argument(
        ...
        param_type = self._map_type(type_ref, type_index)
        json_schema: dict[str, Any] | None = None
        if param_type == "object":
            json_schema = self._build_json_schema(type_ref, type_index)
        elif param_type == "array":
            json_schema = self._build_array_json_schema(type_ref, type_index)
        return Param(
    ...
    def _map_type(self, type_ref: Any, type_index: dict[str, JSONDict]) -> str:
        ...
        if kind == "SCALAR":
            return _SCALAR_TYPE_MAP.get(name) or _infer_scalar_type(name)
        if kind == "ENUM":
            return "string"
    ...
    def _build_json_schema(
        ...
        *,
        ...
        """Build a JSON Schema for a GraphQL INPUT_OBJECT type.

        Returns ``None`` for scalars, enums, or when the type has no
        resolvable input fields.
        """
        if _depth > self._JSON_SCHEMA_MAX_DEPTH or not isinstance(type_ref, dict):
            return None

        named = self._unwrap_named_type(type_ref)
        type_name = str(named.get("name", ""))
        kind = str(named.get("kind", ""))

        # Check type_index for the full definition
        typedef = type_index.get(type_name)
        if typedef is None:
            return None

        actual_kind = typedef.get("kind", kind)

        if actual_kind == "ENUM":
            values = typedef.get("enumValues", [])
            if values:
                enum_names = [v["name"] for v in values if isinstance(v, dict)]
                return {"type": "string", "enum": enum_names}
            return None

        if actual_kind != "INPUT_OBJECT":
            return None

        input_fields = typedef.get("inputFields", [])
        if not input_fields:
            return None
        ...
        for field in input_fields:
            ...
            ftype_ref = field.get("type")
            if not fname:
                continue

            is_req = self._is_required(ftype_ref)
            if is_req:
                required.append(fname)

            mapped = self._map_type(ftype_ref, type_index)
            if mapped == "object":
                child_schema = self._build_json_schema(ftype_ref, type_index, _depth=_depth + 1)
                if child_schema is not None:
                    properties[fname] = child_schema
                    continue

            entry: dict[str, Any] = {"type": mapped}
            fdesc = field.get("description")
            if fdesc:
                entry["description"] = str(fdesc)
            properties[fname] = entry
    ...
    def _build_array_json_schema(
        self,
        type_ref: Any,
        type_index: dict[str, JSONDict],
    ) -> dict[str, Any] | None:
        """Build a JSON Schema for a GraphQL LIST type."""
        if not isinstance(type_ref, dict):
            return None
        # Unwrap NON_NULL / LIST wrappers to find the inner item type
        inner: Any = type_ref
        while isinstance(inner, dict) and inner.get("kind") in ("NON_NULL", "LIST"):
            inner = inner.get("ofType")
        if not isinstance(inner, dict):
            return None
        item_mapped = self._map_type(inner, type_index)
        if item_mapped == "object":
            item_schema = self._build_json_schema(inner, type_index)
            if item_schema is not None:
                return {"type": "array", "items": item_schema}
        return {"type": "array", "items": {"type": item_mapped}}
