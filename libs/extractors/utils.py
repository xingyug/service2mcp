...
def get_auth_headers(source: SourceConfig) -> dict[str, str]:
    ...
    When both ``auth_header`` and ``auth_token`` are provided, ``auth_header``
    is treated as the **header name** (e.g. ``"Authorization"``) and
    ``auth_token`` as the raw **value**.  When only ``auth_header`` is provided
    it is used as the complete ``Authorization`` header value (e.g.
    ``"Bearer tok123"``).  When only ``auth_token`` is provided it is wrapped
