...
class TargetService(BaseModel):
    """Validated target service reference inside a route definition.

    Accepts ``host`` or ``name`` as the service identifier for backward
    compatibility with gateway route payloads that use either key.
    """

    model_config = ConfigDict(extra="allow")

    host: str = Field(min_length=1, validation_alias=AliasChoices("host", "name"))
    port: int = Field(ge=1)
