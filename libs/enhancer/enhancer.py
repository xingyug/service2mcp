...
The enhancer supports parallel batch processing via ``enhance_async()``
(or the ``enhance()`` synchronous wrapper which delegates to the async
implementation when called from an async context).  Concurrency is
controlled by ``EnhancerConfig.max_concurrent_batches`` and defaults
to 5 simultaneous LLM calls.
"""
...
class EnhancerConfig:
    ...
    max_concurrent_batches: int = 5  # parallel LLM calls
    ...
    def from_env(cls) -> EnhancerConfig:
        ...
        max_concurrent_batches = int(_getenv_stripped("LLM_MAX_CONCURRENT_BATCHES") or "5")
...
class TokenUsage:
    ...
    Thread/task-safe: ``add()`` is guarded by an ``asyncio.Lock`` when
    called from ``add_async()``.
    """
    ...
    async def add_async(self, input_tokens: int, output_tokens: int) -> None:
...
class AsyncLLMClient(Protocol):
    """Async interface for LLM API clients."""

    async def complete_async(self, prompt: str, max_tokens: int = 4096) -> LLMResponse:
        """Send a prompt asynchronously and return the response."""
        ...
...
class OpenAILLMClient:
    """LLM client using the OpenAI API (also used for DeepSeek)."""
...
class VertexAILLMClient:
    ...
    async def complete_async(self, prompt: str, max_tokens: int = 4096) -> LLMResponse:
        ...
        return await loop.run_in_executor(None, self.complete, prompt, max_tokens)
...
class IREnhancer:
    ...
    Supports both synchronous (``enhance``) and asynchronous (``enhance_async``)
    execution.  The async path processes batches concurrently up to
    ``config.max_concurrent_batches`` simultaneous LLM calls.
    """
    ...
    def enhance(self, ir: ServiceIR) -> EnhancementResult:
        """Enhance a ServiceIR with improved descriptions (synchronous).
    ...
    async def enhance_async(self, ir: ServiceIR) -> EnhancementResult:
        """Enhance a ServiceIR with parallel LLM calls.

        Processes up to ``config.max_concurrent_batches`` batches
        concurrently using ``asyncio.gather`` + ``asyncio.Semaphore``.
        Requires the client to implement ``complete_async``.
        """
        self.token_usage = TokenUsage(model=self.config.model)
        ...
        batches = self._batch_operations(ops_to_enhance)
        concurrency = max(self.config.max_concurrent_batches, 1)
        semaphore = asyncio.Semaphore(concurrency)

        logger.info(
            "Enhancing %d operations in %d batches (concurrency=%d)",
            len(ops_to_enhance),
            len(batches),
            concurrency,
        )

        async def _process_batch(batch: list[Operation]) -> dict[str, dict[str, Any]]:
            async with semaphore:
                if self.token_usage.total_tokens >= self.config.max_tokens_per_job:
                    logger.warning("Token budget exhausted — skipping batch")
                    return {}
                try:
                    return await self._enhance_batch_async(ir, batch)
                except Exception:
                    logger.warning("LLM enhancement failed for batch", exc_info=True)
                    return {}
    ...
    def _enhance_batch(self, ir: ServiceIR, batch: list[Operation]) -> dict[str, dict[str, Any]]:
        """Call LLM to enhance a batch of operations (synchronous)."""
        prompt = self._build_batch_prompt(ir, batch)
    ...
    async def _enhance_batch_async(
        ...
        """Call LLM to enhance a batch of operations (async)."""
        prompt = self._build_batch_prompt(ir, batch)
        ...
        if isinstance(self.client, AsyncLLMClient):
            response = await self.client.complete_async(prompt, max_tokens=4096)
        else:
            # Fallback: run sync client in thread executor
            loop = asyncio.get_running_loop()
            response = await loop.run_in_executor(None, self.client.complete, prompt, 4096)

        await self.token_usage.add_async(response.input_tokens, response.output_tokens)
        return self._parse_llm_response(response.content)

    def _build_batch_prompt(self, ir: ServiceIR, batch: list[Operation]) -> str:
        """Build the LLM prompt for a batch of operations."""
        ops_json = json.dumps(
            [
                {
                    "operation_id": op.id,
    ...
    def _parse_llm_response(self, content: str) -> dict[str, dict[str, Any]]:
        ...
        # Strategy 1: strip markdown code fences (may appear anywhere)
        ...
        logger.warning("Failed to parse LLM response as JSON array: %s", text[:200])
        return {}
