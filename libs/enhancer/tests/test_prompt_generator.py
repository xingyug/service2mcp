...
class TestRiskAwareActionsPrompt:
    def test_not_generated_when_all_safe(self) -> None:
        ir = _make_ir(
            operations=[
        ...
        prompts = generate_prompts(ir)
        assert not any(p.id.startswith("risk_aware_actions_") for p in prompts)

    def test_generated_with_cautious_ops(self) -> None:
        ir = _make_ir(
            operations=[
                _make_op("list_pets", risk=_make_risk(RiskLevel.safe)),
                _make_op(
        ...
        prompts = generate_prompts(ir)
        risk_prompt = next(
            (p for p in prompts if p.id == "risk_aware_actions_petstore"),
            None,
        )
        assert risk_prompt is not None
        assert "update_pet" in risk_prompt.tool_ids
        assert "list_pets" not in risk_prompt.tool_ids
        assert "risk-aware" in risk_prompt.tags

    def test_destructive_ops_noted_in_template(self) -> None:
        ir = _make_ir(
            operations=[
                _make_op(
                    "delete_pet",
        ...
        prompts = generate_prompts(ir)
        risk_prompt = next(p for p in prompts if p.id == "risk_aware_actions_petstore")
        assert "DESTRUCTIVE" in risk_prompt.template
        assert "delete_pet" in risk_prompt.template

    def test_disabled_risky_ops_excluded(self) -> None:
        ir = _make_ir(
            operations=[
                _make_op(
                    "delete_pet",
