...
def test_parse_llm_response_extracts_json_from_text_with_preamble():
    """Test _parse_llm_response extracts JSON array from mixed text."""
    from libs.enhancer.enhancer import EnhancerConfig, IREnhancer

    client = MockLLMClient()
    enhancer = IREnhancer(client=client, config=EnhancerConfig())

    # DeepSeek often returns text before/after the JSON
    content = (
        "Here are the enhanced descriptions:\n\n```json\n"
        '[{"operation_id": "op1", "description": "Enhanced desc"}]\n'
        "```\n\nI hope this helps!"
    )
    result = enhancer._parse_llm_response(content)
    assert "op1" in result
    assert result["op1"]["description"] == "Enhanced desc"


def test_parse_llm_response_bracket_fallback():
    """Test _parse_llm_response finds JSON array by bracket search."""
    from libs.enhancer.enhancer import EnhancerConfig, IREnhancer

    client = MockLLMClient()
    enhancer = IREnhancer(client=client, config=EnhancerConfig())

    content = 'Sure, here is the result: [{"operation_id": "x", "description": "d"}] done.'
    result = enhancer._parse_llm_response(content)
    assert "x" in result


def test_parse_llm_response_handles_non_dict_enhancement():
...
# ── Async Enhancement Tests ───────────────────────────────────────────────


class AsyncMockLLMClient:
    """Mock LLM client with async support for testing parallel enhancement."""

    def __init__(self, response: str | None = None, fail: bool = False, delay: float = 0.0) -> None:
        self._response = response
        self._fail = fail
        self._delay = delay
        self.calls: list[str] = []
...
class TestAsyncEnhancement:
    """Tests for the async parallel LLM enhancement path."""

    async def test_enhance_async_produces_same_results(self):
        """Async enhancement produces the same result as sync."""
        ir = make_raw_ir(3)
        response = make_llm_response(ir.operations)
        client = AsyncMockLLMClient(response=response)
        config = EnhancerConfig(skip_if_description_exists=False, batch_size=10)
        enhancer = IREnhancer(client=client, config=config)

        result = await enhancer.enhance_async(ir)
        assert result.operations_enhanced == 3
        assert result.enhanced_ir.operations[0].source == SourceType.llm

    async def test_enhance_async_parallel_batches(self):
        """Verify multiple batches are processed concurrently."""
        ir = make_raw_ir(20)  # 20 ops with batch_size=5 → 4 batches
        response = make_llm_response(ir.operations[:5])
        client = AsyncMockLLMClient(response=response, delay=0.05)
        config = EnhancerConfig(
        ...
        await enhancer.enhance_async(ir)
        # All 4 batches should have been called
        assert len(client.calls) == 4
        # Concurrency should be > 1 (batches run in parallel)
        assert client.max_concurrent > 1

    async def test_enhance_async_respects_concurrency_limit(self):
        """Verify semaphore limits concurrent batches."""
        ir = make_raw_ir(30)  # 30 ops with batch_size=5 → 6 batches
        response = make_llm_response(ir.operations[:5])
        client = AsyncMockLLMClient(response=response, delay=0.05)
        config = EnhancerConfig(
            skip_if_description_exists=False,
            batch_size=5,
            max_concurrent_batches=2,  # limit to 2
        )
        enhancer = IREnhancer(client=client, config=config)
    ...
    async def test_enhance_async_handles_batch_failure(self):
        """Async enhancement handles individual batch failures gracefully."""
        call_count = 0
        ...
        class FailOnSecondClient:
            ...
            async def complete_async(self, prompt: str, max_tokens: int = 4096) -> LLMResponse:
                ...
                ir_ops = make_raw_ir(5)
                return LLMResponse(
                    content=make_llm_response(ir_ops.operations),
                    input_tokens=100,
                    output_tokens=50,
                )

        ir = make_raw_ir(15)  # 3 batches of 5
        ...
        result = await enhancer.enhance_async(ir)
        # 2 of 3 batches should succeed
        assert result.operations_enhanced > 0
        assert result.token_usage.total_calls == 2  # 2 successful calls

    async def test_enhance_async_skips_when_no_ops_need_enhancement(self):
        """Async enhancement returns early when nothing needs enhancing."""
        ir = ServiceIR(
            source_hash="abc123",
            protocol="openapi",
            service_name="test-api",
            ...
            operations=[
                Operation(
                    id="op_0",
                    name="well_documented_operation",
                    description="This operation is very well documented already",
                    method="GET",
                    path="/endpoint",
                    params=[
                        Param(
    ...
    async def test_enhance_async_token_budget_exhaustion(self):
        """Async enhancement respects token budget across concurrent batches."""
        ir = make_raw_ir(20)
        response = make_llm_response(ir.operations[:5])
        client = AsyncMockLLMClient(response=response)
        config = EnhancerConfig(
            ...
            batch_size=5,
            max_concurrent_batches=4,
            max_tokens_per_job=100,  # very low — exhausted after ~1 batch
        )
        enhancer = IREnhancer(client=client, config=config)

        result = await enhancer.enhance_async(ir)
        # Token budget should limit how many batches produce results
        assert result.token_usage.total_calls <= 4

    async def test_enhance_async_fallback_to_sync_client(self):
        """Async enhancement works with sync-only clients via executor."""
        ir = make_raw_ir(3)
        response = make_llm_response(ir.operations)
        # Use the sync-only MockLLMClient (no complete_async)
...
class TestEnhancerConfigConcurrency:
    """Tests for max_concurrent_batches configuration."""

    def test_default_concurrency(self):
        config = EnhancerConfig()
        assert config.max_concurrent_batches == 5

    def test_from_env_reads_concurrency(self, monkeypatch):
        monkeypatch.setenv("LLM_API_KEY", "test-key")
        monkeypatch.setenv("LLM_MAX_CONCURRENT_BATCHES", "10")
        config = EnhancerConfig.from_env()
        assert config.max_concurrent_batches == 10

    def test_from_env_default_concurrency(self, monkeypatch):
        monkeypatch.setenv("LLM_API_KEY", "test-key")
        monkeypatch.delenv("LLM_MAX_CONCURRENT_BATCHES", raising=False)
        config = EnhancerConfig.from_env()
        assert config.max_concurrent_batches == 5


class TestTokenUsageAsync:
    """Tests for async-safe token usage tracking."""

    async def test_add_async_tracks_correctly(self):
        usage = TokenUsage(model="test")
        await usage.add_async(100, 50)
        await usage.add_async(200, 100)
        assert usage.total_tokens == 450
        assert usage.total_calls == 2

    async def test_concurrent_add_async_is_safe(self):
        import asyncio

        usage = TokenUsage(model="test")
        tasks = [usage.add_async(10, 5) for _ in range(100)]
        await asyncio.gather(*tasks)
        assert usage.total_tokens == 1500  # 100 * (10 + 5)
