...
def generate_prompts(ir: ServiceIR) -> list[PromptDefinition]:
    ...
    risk_prompt = _risk_aware_actions_prompt(ir)
...
def _risk_aware_actions_prompt(ir: ServiceIR) -> PromptDefinition | None:
    """Generate a prompt for cautious/dangerous tools requiring confirmation.

    Returns None if the service has no risky operations.
    """
    risky_ops = [
        op
        for op in ir.operations
        if op.enabled and op.risk.risk_level in (RiskLevel.cautious, RiskLevel.dangerous)
    ]
    if not risky_ops:
        return None

    risky_ids = [op.id for op in risky_ops]
    destructive_ids = [op.id for op in risky_ops if op.risk.destructive]

    tool_summary = ", ".join(risky_ids)
    caution = ""
    if destructive_ids:
        destructive_list = ", ".join(destructive_ids)
        caution = f" DESTRUCTIVE tools ({destructive_list}) require explicit user confirmation."

    return PromptDefinition(
        id=f"risk_aware_actions_{ir.service_name}",
        name=f"Risk-aware actions {ir.service_name}",
        description=f"Use cautious/dangerous tools on {ir.service_name} with explicit confirmation",
        template=(
            "You may use the following risky tools on {service_name}, "
            "but ALWAYS confirm with the user before executing: " + tool_summary + "." + caution
        ),
        arguments=[
            PromptArgument(
                name="service_name",
