...
async def create_policy(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail="Gateway sync failed after policy creation.",
        ) from exc
    try:
...
async def update_policy(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Gateway sync failed after policy update.",
        ) from exc
    try:
        await audit_log.append_entry(
            actor=caller.subject,
...
async def delete_policy(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="Gateway sync failed after policy deletion.",
        ) from exc
    try:
        await audit_log.append_entry(
            actor=caller.subject,
...
async def evaluate_policy(
    ...
    try:
        ...
        logger.exception("Audit log failed during policy evaluation")
    return result
