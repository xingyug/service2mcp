"""Database wiring for the access control service."""
...
def build_engine_and_session_factory(
    ...
    """Create the SQLAlchemy async engine and session factory for the service."""
...
def resolve_session_factory(app: FastAPI) -> async_sessionmaker[AsyncSession]:
    ...
    if not database_url:
        raise RuntimeError("DATABASE_URL must be configured for access_control database access.")
