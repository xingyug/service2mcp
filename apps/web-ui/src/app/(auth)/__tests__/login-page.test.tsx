...
describe("LoginPage", () => {
  ...
  it("renders page heading and description", () => {
    render(<LoginPage />);
    expect(screen.getByText("service2mcp")).toBeInTheDocument();
    expect(
  ...
  it("clears error when switching between JWT and PAT tabs", async () => {
    const user = userEvent.setup();
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
      mockApiErrorResponse("JWT must contain three segments."),
    );

    render(<LoginPage />);

    // Trigger an error on JWT tab
    const jwtInput = screen.getByLabelText("JWT Token", { selector: "input" });
    await user.type(jwtInput, "bad-token");
    await user.click(screen.getByRole("button", { name: "Sign in with JWT" }));

    await waitFor(() => {
      expect(screen.getByText("JWT must contain three segments.")).toBeInTheDocument();
    });

    // Switch to PAT tab — error should clear
    await user.click(screen.getByRole("tab", { name: "PAT Token" }));

    await waitFor(() => {
      expect(screen.queryByText("JWT must contain three segments.")).not.toBeInTheDocument();
    });
  });
});
