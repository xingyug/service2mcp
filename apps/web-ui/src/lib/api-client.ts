...
const ACCESS_CONTROL_API =
  process.env.NEXT_PUBLIC_ACCESS_CONTROL_URL || "";
...
function createEventSource(url: string): EventSource {
  ...
  // so we must pass the auth token via the query string.  This exposes the
  // token in browser history, proxy logs, and devtools.  To limit risk,
  // use SHORT-LIVED tokens (≤ 5 min) for SSE connections.  The backend
  // logs a warning when a long-lived token is detected in the query string.
  const token = getAuthToken();
...
function normalizeCompilationJob(
  ...
  const artifacts =
    rawArtifacts != null
      ? {
          ir_id: rawArtifacts.ir_id ?? undefined,
          image_digest: rawArtifacts.image_digest ?? undefined,
          deployment_id: rawArtifacts.deployment_id ?? undefined,
        }
      : serviceId
        ? { ir_id: serviceId }
        : undefined;
...
export const serviceApi = {
  ...
  delete(serviceId: string, scope?: ServiceScope) {
