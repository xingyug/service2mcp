...
# ── _json_safe_value edge cases ──────────────────────────────────────────


class TestJsonSafeValueEdgeCases:
    """Tests for _json_safe_value handling of bytes, float('inf'), float('nan')."""

    def test_bytes_utf8_decoded(self) -> None:
        assert _json_safe_value(b"hello") == "hello"

    def test_bytes_binary_base64_encoded(self) -> None:
        result = _json_safe_value(b"\x00\x01\xff\xfe")
        assert result.startswith("base64:")
        import base64

        decoded = base64.b64decode(result.removeprefix("base64:"))
        assert decoded == b"\x00\x01\xff\xfe"

    def test_float_inf_to_string(self) -> None:
        assert _json_safe_value(float("inf")) == "inf"

    def test_float_neg_inf_to_string(self) -> None:
        assert _json_safe_value(float("-inf")) == "-inf"

    def test_float_nan_to_string(self) -> None:
        assert _json_safe_value(float("nan")) == "nan"

    def test_normal_float_unchanged(self) -> None:
        assert _json_safe_value(3.14) == 3.14

    def test_json_safe_row_with_bytes_and_inf(self) -> None:
        import json

        row = {"data": b"\x80\x81", "score": float("inf"), "name": "test"}
        result = _json_safe_row(row)
        # Must be JSON-serializable without errors
        serialized = json.dumps(result)
        assert "base64:" in serialized
        assert '"inf"' in serialized
