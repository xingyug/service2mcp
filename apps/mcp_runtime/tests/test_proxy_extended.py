...
class TestBuildPrimaryAuth:
    ...
    async def test_unsupported_auth_type_raises(self) -> None:
        ...
        p._service_ir.auth.type = "unknown_type"
...
class TestFetchOAuth2AccessToken:
    async def test_cached_token_returned(self) -> None:
        ...
        p = _proxy()
        cache_key = "\x00".join([oauth2.token_url, oauth2.client_id_ref or "", "read", ""])
    ...
    async def test_expired_token_refetched(self) -> None:
        ...
        p = _proxy()
        cache_key = "\x00".join([oauth2.token_url, oauth2.client_id_ref or "", "", ""])
    ...
    async def test_connect_error_raises_tool_error(self) -> None:
        oauth2 = OAuth2ClientCredentialsConfig(
            token_url="https://auth.example.com/token",
            client_id_ref="CID",
            client_secret_ref="CSEC",
    ...
    async def test_http_status_error_raises_tool_error(self) -> None:
        oauth2 = OAuth2ClientCredentialsConfig(
            token_url="https://auth.example.com/token",
            client_id_ref="CID",
            client_secret_ref="CSEC",
        ...
        with patch.dict("os.environ", {"CID": "id", "CSEC": "sec"}):
            with pytest.raises(ToolError, match="status 401"):
                await p._fetch_oauth2_access_token("op1", oauth2)
...
class TestPollAsyncJob:
    ...
    async def test_invalid_json_poll_raises_immediately(self) -> None:
        """invalid JSON in poll response must raise immediately,
...
class TestResolveSecretValue:
    ...
    def test_missing_env_var_raises(self) -> None:
        with patch.dict("os.environ", {}, clear=True):
            with pytest.raises(ToolError, match="Failed to resolve test secret"):
...
class TestNormalizeWebsocketMessage:
    ...
    def test_invalid_base64_raises_tool_error(self) -> None:
        """bad base64 must raise ToolError, not silently decode."""
...
class TestSanitizeJsonFloats:
    """Tests for _sanitize_json_floats in proxy_utils."""

    def test_nan_replaced_with_string(self) -> None:
        from apps.mcp_runtime.proxy_utils import _sanitize_json_floats

        result = _sanitize_json_floats(float("nan"))
        assert isinstance(result, str)
        assert result == "nan"

    def test_inf_replaced_with_string(self) -> None:
        from apps.mcp_runtime.proxy_utils import _sanitize_json_floats

        result = _sanitize_json_floats(float("inf"))
        assert isinstance(result, str)
        assert result == "inf"

    def test_neg_inf_replaced_with_string(self) -> None:
        from apps.mcp_runtime.proxy_utils import _sanitize_json_floats
    ...
    def test_normal_float_unchanged(self) -> None:
        ...
        assert _sanitize_json_floats(3.14) == 3.14

    def test_nested_dict_with_nan(self) -> None:
        from apps.mcp_runtime.proxy_utils import _sanitize_json_floats

        data = {"a": 1, "b": float("nan"), "c": {"d": float("inf")}}
        result = _sanitize_json_floats(data)
        assert result["a"] == 1
        assert result["b"] == "nan"
        assert result["c"]["d"] == "inf"

    def test_list_with_non_finite(self) -> None:
        from apps.mcp_runtime.proxy_utils import _sanitize_json_floats

        data = [1.0, float("nan"), float("-inf"), "hello"]
        result = _sanitize_json_floats(data)
        assert result == [1.0, "nan", "-inf", "hello"]

    def test_response_payload_sanitizes_nan(self) -> None:
        """End-to-end: _parse_response_payload sanitizes NaN in JSON response."""
        from apps.mcp_runtime.proxy import _parse_response_payload

        response = httpx.Response(
            200,
            text='{"value": NaN, "count": Infinity}',
            headers={"content-type": "application/json"},
            request=_make_request(),
        )
        result = _parse_response_payload(response)
        assert result["value"] == "nan"
        assert result["count"] == "inf"
        # Verify result is now JSON-serializable with strict mode
...
class TestInvokeIntegration:
    ...
    async def test_invoke_tool_error_from_sql_records_failure(self) -> None:
        ...
        with pytest.raises(ToolError, match="SQL failed"):
            await p.invoke(op, {})
        # The breaker should have recorded a failure via the ToolError handler
        assert op.id in p.breakers
        assert p.breakers[op.id].consecutive_failures >= 1
...
class TestPollAsyncJobDeadlineTimeout:
    """Ensure polling per-request timeout shrinks toward the deadline."""

    async def test_polling_timeout_decreases_toward_deadline(self) -> None:
        from apps.mcp_runtime.proxy_http import poll_async_job

        captured_timeouts: list[float] = []

        async def fake_send(**kw: Any) -> httpx.Response:
            captured_timeouts.append(kw.get("timeout", 0.0))
            return httpx.Response(
                200,
                json={"status": "complete"},
                request=httpx.Request("GET", "http://poll"),
            )

        config = AsyncJobConfig(
            initial_status_codes=[202],
            status_url_source="location_header",
            success_status_values=["complete"],
            timeout_seconds=5,
            poll_interval_seconds=0.01,
        )
        initial_response = httpx.Response(
            202,
            headers={"Location": "http://api/status"},
            request=httpx.Request("POST", "http://api/start"),
        )
        ir = ServiceIR(
            service_name="svc",
            base_url="http://t:80",
        ...
        await poll_async_job(
            ...
            get_client=lambda: AsyncMock(),
            timeout=30.0,
            send_fn=fake_send,
        )

        assert len(captured_timeouts) >= 1
        # The effective timeout should be <= min(30.0, remaining_deadline)
        # With a 5-second deadline, the timeout should be at most ~5s
        assert captured_timeouts[0] <= 5.1


# ── Event bridge error wrapping ──────────────────────────────────────────


class TestEventBridgeErrorWrapping:
    """Verify broker exceptions in _perform_event_bridge are wrapped in ToolError."""

    @pytest.mark.asyncio
    async def test_publish_connection_error_wrapped_in_tool_error(self) -> None:
        from libs.ir.models import (
            EventBridgeConfig,
            EventDescriptor,
            EventDirection,
        ...
        op = Operation(id="pub_test", name="pub_test")
        ed = EventDescriptor(
            id="pub_test",
            name="pub_test",
            ...
            event_bridge=EventBridgeConfig(broker_url="kafka://localhost:9092", topic="test-topic"),
            operation_id="pub_test",
        )
        ir = ServiceIR(
            service_name="test",
            protocol="rest",
            ...
            source_hash="abc",
            operations=[op],
            event_descriptors=[ed],
        )

        mock_bridge = AsyncMock()
        mock_bridge.publish = AsyncMock(side_effect=ConnectionError("broker down"))

        obs = RuntimeObservability()
        proxy = RuntimeProxy(service_ir=ir, event_bridge_client=mock_bridge, observability=obs)
        with pytest.raises(ToolError, match="Event bridge error.*broker down"):
            await proxy._perform_event_bridge(op, {"key": "val"}, ed)

    @pytest.mark.asyncio
    async def test_observe_runtime_error_wrapped_in_tool_error(self) -> None:
        from libs.ir.models import (
            EventDescriptor,
            EventDirection,
            EventSupportLevel,
        ...
        op = Operation(id="obs_test", name="obs_test")
        ed = EventDescriptor(
            id="obs_test",
            name="obs_test",
            transport=EventTransport.kafka,
        ...
        mock_bridge = AsyncMock()
        mock_bridge.observe = AsyncMock(side_effect=RuntimeError("consumer died"))

        obs = RuntimeObservability()
        proxy = RuntimeProxy(service_ir=ir, event_bridge_client=mock_bridge, observability=obs)
        with pytest.raises(ToolError, match="Event bridge error.*consumer died"):
            await proxy._perform_event_bridge(op, {}, ed)

    @pytest.mark.asyncio
    async def test_tool_error_from_bridge_not_double_wrapped(self) -> None:
        from libs.ir.models import (
            EventBridgeConfig,
            EventDescriptor,
            EventDirection,
        ...
        op = Operation(id="te_test", name="te_test")
        ed = EventDescriptor(
            id="te_test",
            name="te_test",
            transport=EventTransport.kafka,
            support=EventSupportLevel.supported,
            direction=EventDirection.outbound,
            event_bridge=EventBridgeConfig(broker_url="kafka://localhost:9092", topic="t"),
            operation_id="te_test",
        )
        ir = ServiceIR(
            service_name="test",
            protocol="rest",
        ...
        mock_bridge = AsyncMock()
        mock_bridge.publish = AsyncMock(side_effect=ToolError("not connected"))
