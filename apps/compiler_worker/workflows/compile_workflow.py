...
_TERMINAL_STATUSES = frozenset(
    {CompilationStatus.SUCCEEDED, CompilationStatus.FAILED, CompilationStatus.ROLLED_BACK}
)
_NON_REENTRANT_STATUSES = _TERMINAL_STATUSES | {CompilationStatus.RUNNING}
...
class CompilationWorkflow:
    ...
    async def run(self, request: CompilationRequest) -> CompilationResult:
        ...
        existing_status = await self._store.get_job_status(job_id)
        if existing_status is not None and existing_status in _NON_REENTRANT_STATUSES:
            logger.warning(
                "Job %s already has status %s — skipping duplicate execution.",
                job_id,
        ...
        for stage_definition in stage_definitions:
            ...
            await self._store.mark_job_running(
                ...
                service_id=context.service_name,
            )
