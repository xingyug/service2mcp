...
class ProductionActivitySettings:
    ...
    def from_env(cls) -> ProductionActivitySettings:
        ...
        # Parse comma-separated image pull secret names
        raw_secrets = os.getenv("MCP_RUNTIME_IMAGE_PULL_SECRETS", "")
        image_pull_secrets = [s.strip() for s in raw_secrets.split(",") if s.strip()]

        # Parse node selector as comma-separated key=value pairs
        raw_ns = os.getenv("MCP_RUNTIME_NODE_SELECTOR", "")
        node_selector: dict[str, str] = {}
        for pair in raw_ns.split(","):
            pair = pair.strip()
            if "=" in pair:
                k, v = pair.split("=", 1)
...
class DeferredRoutePublisher:
    ...
    async def publish(self, route_config: dict[str, Any]) -> dict[str, Any] | None:
        default_route = route_config.get("default_route")
        version_route = route_config.get("version_route")
        return {
            "mode": self.mode,
            "default_route_id": (
                default_route.get("route_id") if isinstance(default_route, dict) else None
            ),
            "version_route_id": (
                version_route.get("route_id") if isinstance(version_route, dict) else None
            ),
        }
        ...
        *,
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                raise RuntimeError(
                    f"Access control route publisher returned {response.status_code} "
                    f"for {path}: {body_snippet}"
                )
            try:
...
class KubernetesManifestDeployer:
    ...
    async def deploy(self, manifest_set: GeneratedManifestSet) -> DeploymentResult:
        ...
        except Exception as exc:  # broad-except: partial K8s manifest cleanup
            _logger.error(
                "Kubernetes deployment failed (%s): %s",
                type(exc).__name__,
                exc,
                exc_info=True,
            )
    ...
    async def _apply_manifest(
        ...
        if response.status_code == 404:
            ...
            if response.status_code == 409:
                # Resource created concurrently; retry as PATCH
                response = await self.api.client.patch(
                    named_path,
                    json=manifest,
                    headers={"Content-Type": "application/merge-patch+json"},
...
def create_default_activity_registry(
    *,
    ...
    async def detect_stage(context: CompilationContext) -> StageExecutionResult:
        ...
        forced = source.hints.get("protocol")
        extractors = _build_extractors()
        try:
            if forced:
                # User-supplied force_protocol overrides auto-detection.
                return _stage_result(
                    context_updates={"detection_confidence": 1.0},
                    event_detail={"confidence": 1.0, "forced": True},
                    protocol=forced,
                )
    ...
    async def enhance_stage(context: CompilationContext) -> StageExecutionResult:
        ...
        result = await enhancer.enhance_async(service_ir)
        enhanced_ir = await _apply_post_enhancement_async(
            result.enhanced_ir,
            llm_client_factory=lambda: create_llm_client(enhancer_config),
            max_concurrency=enhancer_config.max_concurrent_batches,
        )
        token_usage = {
    ...
    async def validate_ir_stage(context: CompilationContext) -> StageExecutionResult:
        ...
        if not report.overall_passed:
            raise RuntimeError(_validation_failure_message("Pre-deploy validation failed", report))
        return _stage_result(
        ...
        if runtime_mode == "codegen":
            manifest_set = generate_codegen_manifests(
                ...
                config=CodegenManifestConfig(
                    ...
                    image_pull_secrets=resolved_settings.image_pull_secrets,
                    node_selector=resolved_settings.node_selector,
                ),
            )
        else:
...
def _resolve_extractor(
    ...
    # User-supplied hint (force_protocol) takes priority over auto-detection.
    protocol = source.hints.get("protocol") or context.protocol
    if protocol:
...
def _apply_post_enhancement(
    ...
    *,
    ...
    # Resource and prompt auto-generation: always run (deterministic, no LLM needed)
    if not ir.resource_definitions:
        ir = ir.model_copy(update={"resource_definitions": generate_resources(ir)})
    if not ir.prompt_definitions:
        ir = ir.model_copy(update={"prompt_definitions": generate_prompts(ir)})
...
async def _apply_post_enhancement_async(
    ir: ServiceIR,
    *,
    llm_client_factory: Callable[[], Any] | None = None,
    max_concurrency: int = 5,
) -> ServiceIR:
    """Async version of post-enhancement with parallel examples generation."""
    from libs.enhancer.error_normalizer import normalize_error_schemas
    from libs.enhancer.examples_generator import ExamplesGenerator
    from libs.enhancer.prompt_generator import generate_prompts
    from libs.enhancer.resource_generator import generate_resources
    ...
    if llm_client_factory is not None:
        try:
            generator = ExamplesGenerator(llm_client_factory(), max_concurrency=max_concurrency)
            ir = await generator.generate_async(ir)
        except Exception:
            import logging
        ...
        for attempt in range(1 + max_retries):
            try:
                client = (
                    ...
                    else httpx.AsyncClient(follow_redirects=True, timeout=60.0)
                )
                async with client:
                ...
                if result.isError:
                    # isError=True means the MCP runtime processed the request
                    # but the upstream returned a protocol-level error (SOAP fault,
                    # JSONRPC error, etc.).  This still proves end-to-end connectivity.
                    return {"status": "upstream_error", "error": result.content}
                structured = result.structuredContent
                if isinstance(structured, dict):
                    return structured
                return {"status": "ok", "result": structured}
            except (httpx.ConnectError, httpx.ReadError, httpx.WriteError, OSError) as exc:
                last_exc = exc
