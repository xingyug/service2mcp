...
class TestApplyPostEnhancement:
    ...
    def test_populates_resource_definitions_when_empty(self) -> None:
        ir = _ir(operations=[_op("list_pets", method="GET")])
        assert ir.resource_definitions == []
        with patch.dict(os.environ, {"WORKER_ENABLE_TOOL_GROUPING": ""}):
            result = _apply_post_enhancement(ir)
        assert len(result.resource_definitions) > 0

    def test_populates_prompt_definitions_when_empty(self) -> None:
        ir = _ir(operations=[_op("list_pets", method="GET")])
        assert ir.prompt_definitions == []
        with patch.dict(os.environ, {"WORKER_ENABLE_TOOL_GROUPING": ""}):
            result = _apply_post_enhancement(ir)
        assert len(result.prompt_definitions) > 0

    def test_does_not_overwrite_existing_resource_definitions(self) -> None:
        from libs.ir.models import ResourceDefinition
        ...
        ir = _ir(operations=[_op("list_pets", method="GET")])
        ir = ir.model_copy(update={"resource_definitions": [existing]})
        with patch.dict(os.environ, {"WORKER_ENABLE_TOOL_GROUPING": ""}):
            result = _apply_post_enhancement(ir)
        assert len(result.resource_definitions) == 1
        assert result.resource_definitions[0].id == "custom-res"

    def test_does_not_overwrite_existing_prompt_definitions(self) -> None:
        from libs.ir.models import PromptDefinition

        existing = PromptDefinition(
            id="custom-prompt",
            name="Custom prompt",
            description="Operator-supplied",
            template="Do something with {service_name}",
        )
        ir = _ir(operations=[_op("list_pets", method="GET")])
        ir = ir.model_copy(update={"prompt_definitions": [existing]})
        with patch.dict(os.environ, {"WORKER_ENABLE_TOOL_GROUPING": ""}):
            result = _apply_post_enhancement(ir)
        assert len(result.prompt_definitions) == 1
        assert result.prompt_definitions[0].id == "custom-prompt"
...
class TestDeferredRoutePublisher:
    ...
    @pytest.mark.asyncio
    async def test_publish_version_only_no_default_route(self) -> None:
        """publish() must not crash when default_route is None."""
        from apps.compiler_worker.activities.production import DeferredRoutePublisher

        publisher = DeferredRoutePublisher()
        route_config = {
            "service_id": "my-svc",
            "service_name": "my-svc-v2",
            "namespace": "default",
            "default_route": None,
            "version_route": {"route_id": "my-svc-v2"},
        }
        result = await publisher.publish(route_config)
        assert result is not None
        assert result["default_route_id"] is None
        assert result["version_route_id"] == "my-svc-v2"
...
class TestAccessControlRoutePublisherPost:
    ...
    async def test_http_error_includes_response_body(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="422.*validation error"):
            await publisher._post("/test", route_config={"key": "value"})
    ...
    async def test_rollback_rejects_non_object_publication(self) -> None:
        ...
        with pytest.raises(RuntimeError, match="publication must be an object"):
            await publisher.rollback({"default_route": {"route_id": "r1"}}, "published")
...
class TestRuntimeReadinessTimeouts:
    ...
    async def test_wait_for_runtime_http_ready_handles_client_factory_error(self) -> None:
        from apps.compiler_worker.activities.production import _wait_for_runtime_http_ready

        def failing_factory(_: str) -> None:
