...
class TestResolveCompilationExecutor:
    ...
    def test_caches_executor_across_calls(self) -> None:
        with patch.dict(os.environ, {"DATABASE_URL": "postgresql+asyncpg://u:p@h/db"}):
            first = resolve_compilation_executor()
            second = resolve_compilation_executor()
...
class TestDatabaseWorkflowCompilationExecutor:
    ...
    @pytest.mark.asyncio
    async def test_execute_creates_fresh_engine_per_run(self) -> None:
        ...
        with (
            ...
            patch("apps.compiler_worker.executor.async_sessionmaker"),
            patch("apps.compiler_worker.executor.SQLAlchemyCompilationJobStore"),
            patch("apps.compiler_worker.executor.CompilationWorkflow") as mock_wf_cls,
            patch("apps.compiler_worker.executor.create_default_activity_registry"),
        ):
            mock_workflow = AsyncMock()
