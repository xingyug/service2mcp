...
class TestToJobRecord:
    def test_basic_fields(self) -> None:
        job = _fake_job()
        record = _store._to_job_record(job)
        ...
        assert record.service_id == "test-api"
    ...
    def test_service_id_none(self) -> None:
        job = _fake_job(service_id=None)
        record = _store._to_job_record(job)
...
class TestToEventRecord:
    def test_basic_fields(self) -> None:
        event = _fake_event()
        record = _store._to_event_record(event)
...
class TestUpdateCheckpointSourceHash:
    """Test that update_checkpoint writes source_hash back to the job."""

    @pytest.mark.asyncio
    async def test_source_hash_written_from_payload(self) -> None:
        from unittest.mock import AsyncMock, MagicMock, patch

        from apps.compiler_worker.models import CompilationStage
        ...
        with patch(
            "apps.compiler_worker.repository.store_compilation_checkpoint",
            return_value={"key": "value", "checkpoint": True},
        ):
            await store.update_checkpoint(
                fake_job.id,
                payload={"source_hash": "sha256:deadbeef", "other": "data"},
                protocol="openapi",
                service_name="test-svc",
                completed_stage=CompilationStage.EXTRACT,
            )

        assert fake_job.source_hash == "sha256:deadbeef"

    @pytest.mark.asyncio
    async def test_source_hash_not_overwritten_when_absent(self) -> None:
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        assert fake_job.source_hash == "sha256:original"
