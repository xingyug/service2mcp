...
class FakeJobStore:
    ...
    async def get_job_status(self, job_id: UUID) -> CompilationStatus | None:
        ...
        if job is None:
            return None
        return CompilationStatus(job["status"])
...
class TestCompilationWorkflowDuplicateJob:
    @pytest.mark.asyncio
    async def test_skips_execution_when_job_already_succeeded(self) -> None:
        store = FakeJobStore()
        activities = FakeActivities()
        stages = (StageDefinition(stage=CompilationStage.DETECT),)

        job_id = uuid4()
        store.jobs[job_id] = {"status": "succeeded", "options": {}}

        wf = CompilationWorkflow(store=store, activities=activities, stage_definitions=stages)
        request = CompilationRequest(source_url="https://example.com", job_id=job_id)
        result = await wf.run(request)

        assert result.status == CompilationStatus.SUCCEEDED
        assert result.job_id == job_id
        assert activities.run_calls == []
        assert store.events == []

    @pytest.mark.asyncio
    async def test_skips_execution_when_job_already_failed(self) -> None:
        store = FakeJobStore()
        activities = FakeActivities()
        stages = (StageDefinition(stage=CompilationStage.DETECT),)

        job_id = uuid4()
        store.jobs[job_id] = {"status": "failed", "options": {}}
    ...
    @pytest.mark.asyncio
    async def test_skips_execution_when_job_already_running(self) -> None:
        store = FakeJobStore()
        activities = FakeActivities()
        stages = (StageDefinition(stage=CompilationStage.DETECT),)

        job_id = uuid4()
        store.jobs[job_id] = {"status": "running", "options": {}}

        wf = CompilationWorkflow(store=store, activities=activities, stage_definitions=stages)
        request = CompilationRequest(source_url="https://example.com", job_id=job_id)
        result = await wf.run(request)

        assert result.status == CompilationStatus.RUNNING
        assert activities.run_calls == []

    @pytest.mark.asyncio
    async def test_executes_normally_for_pending_job(self) -> None:
        store = FakeJobStore()
        activities = FakeActivities()
        stages = (StageDefinition(stage=CompilationStage.DETECT),)
        ...
        assert result.status == CompilationStatus.SUCCEEDED
        assert CompilationStage.DETECT in activities.run_calls
