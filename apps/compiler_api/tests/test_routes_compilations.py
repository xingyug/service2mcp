...
class TestListCompilations:
    async def test_returns_repository_jobs(self) -> None:
        ...
        with patch(
            ...
            mock_repo.list_jobs.assert_called_once_with(service_id=None)

    async def test_filters_by_service_id(self) -> None:
        mock_session = AsyncMock()
        mock_jobs = [MagicMock(spec=CompilationJobResponse)]
        ...
        with patch(
            ...
            result = await list_compilations(mock_session, service_id="my-service")

            assert result == mock_jobs
            mock_repo.list_jobs.assert_called_once_with(service_id="my-service")
...
class TestRetryCompilation:
    """POST /api/v1/compilations/{jobId}/retry must exist."""
...
class TestRollbackCompilation:
    """POST /api/v1/compilations/{jobId}/rollback."""
