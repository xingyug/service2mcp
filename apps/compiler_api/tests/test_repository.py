...
class TestListJobs:
    ...
    async def test_list_jobs_service_id_filter(self) -> None:
        jobs = [_fake_job(service_id="my-svc")]
        mock_session = AsyncMock()
        mock_result = MagicMock()
        mock_result.all.return_value = jobs
        mock_session.scalars.return_value = mock_result
        ...
        repo = CompilationRepository(mock_session)
        result = await repo.list_jobs(service_id="my-svc")

        assert len(result) == 1
        assert result[0].service_id == "my-svc"

    async def test_to_job_response_maps_service_id(self) -> None:
        job = _fake_job(service_id="canonical-id", service_name="Human Name")
        response = CompilationRepository._to_job_response(job)
        assert response.service_id == "canonical-id"
        assert response.service_name == "Human Name"

    async def test_to_job_response_service_id_none(self) -> None:
        job = _fake_job(service_id=None, service_name="My API")
        response = CompilationRepository._to_job_response(job)
        assert response.service_id is None
        assert response.service_name == "My API"
...
class TestToResponseEdgeCases:
    ...
    def test_with_all_optional_fields_populated(self) -> None:
        ...
        result = ArtifactRegistryRepository._to_response(version)
...
class TestToEventResponseEdgeCases:
    ...
    def test_event_with_error_detail(self) -> None:
        ...
        result = CompilationRepository._to_event_response(event)
