...
from apps.access_control.security import caller_roles, require_authenticated_caller
...
_TRANSITION_REQUIRED_ROLES: dict[tuple[str, str], set[str]] = {
    ("in_review", "approved"): {"reviewer", "admin"},
    ("approved", "published"): {"publisher", "admin"},
    ("published", "deployed"): {"deployer", "admin"},
}
...
async def transition_workflow(
    ...
    required = _TRANSITION_REQUIRED_ROLES.get((record.state, payload.to))
    if required is not None:
        roles = caller_roles(caller)
        if not roles & required:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail=(
                    f"Transition from '{record.state}' to '{payload.to}' "
                    f"requires one of {sorted(required)} roles."
                ),
            )
