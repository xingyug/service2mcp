    ...
    async def list_jobs(
        ...
        *,
        ...
        if service_id is not None:
            query = query.where(CompilationJob.service_id == service_id)
...
class ServiceCatalogRepository:
    ...
    async def get_dashboard_summary(
        ...
        *,
        ...
        service_list = await self.list_services(tenant=tenant, environment=environment)

        total_tools = sum(s.tool_count for s in service_list.services)
        protocol_dist: dict[str, int] = {}
        for svc in service_list.services:
            proto = svc.protocol or "unknown"
        ...
        result = await self._session.execute(query.limit(recent_limit))
        recent = [
            CompilationJobResponse(
                id=job.id,
        ...
        return DashboardSummaryResponse(
            total_services=len(service_list.services),
...
class ArtifactRegistryRepository:
    ...
    async def delete_service(
        ...
        *,
        ...
        """Delete ALL versions of a service. Returns number of versions deleted."""
        query = select(ServiceVersion).where(ServiceVersion.service_id == service_id)
        if tenant is not None:
            query = query.where(ServiceVersion.tenant == tenant)
        if environment is not None:
            query = query.where(ServiceVersion.environment == environment)
